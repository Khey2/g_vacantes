...
mysql_select_db($database_vacantes, $vacantes);
$query_matriz = "SELECT count( prod_activosfaltas.IDempleado ) AS Totales, prod_activosfaltas.IDempleado, prod_activosfaltas.emp_paterno, prod_activosfaltas.emp_materno, prod_activosfaltas.emp_nombre, prod_activosfaltas.denominacion, prod_activosfaltas.IDmatriz, prod_activosfaltas.fecha_alta, prod_activosfaltas.IDarea, prod_activosfaltas.fecha_baja, vac_puestos.sgmm,  prod_activosfaltas.descripcion_nomina, vac_matriz.matriz, vac_areas.area FROM prod_activosfaltas LEFT JOIN vac_puestos ON prod_activosfaltas.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_matriz ON prod_activosfaltas.IDmatriz = vac_matriz.IDmatriz
LEFT JOIN vac_areas ON prod_activosfaltas.IDarea = vac_areas.IDarea WHERE ( MONTH ( prod_activosfaltas.fecha_baja ) = '$el_mes' AND YEAR ( prod_activosfaltas.fecha_baja ) = '$anio' AND vac_puestos.sgmm = 1 ) OR ( MONTH ( prod_activosfaltas.fecha_alta ) = '$el_mes' AND YEAR ( prod_activosfaltas.fecha_alta ) = '$anio' AND vac_puestos.sgmm = 1 ) GROUP BY prod_activosfaltas.IDempleado"; 
...
$mail->AddAddress('lguevara@sahuayo.mx');
$mail->AddAddress('gemendiola@sahuayo.mx');
$mail->AddAddress('dmmartinez@sahuayo.mx');
$mail->AddAddress('feavalos@sahuayo.mx');
$mail->AddAddress('sgruiz@sahuayo.mx');
$mail->Subject = 'Reporte Mensual bajas SGMM';
$mail->isHTML(true);
$mail->CharSet = 'UTF-8';
$mail->AltBody = 'Reporte Mensual bajas SGMM';
...
<tr>
  <td align="center" style="color: #343434; font-size: 24px; font-family: Quicksand, Calibri, sans-serif; font-weight:700;letter-spacing: 3px; line-height: 35px;" class="main-header">
        <!-- section text ======-->
        <div style="line-height: 35px">Reporte de Altas y Bajas <span style="color: #d8020a;">SGMM</span> <br />'.$elmes.'</div>
    </td>
</tr>
<tr>
...
<td align="left" style="color: #888888; font-size: 16px; font-family: "Work Sans", Calibri, sans-serif; line-height: 24px;">
	...
	A continuación se muestran las bajas y altas de Empleados que les aplica el SGMM.<br />
	El reporte puede incluir promociones y se genera de forma automática cada inicio de mes.<br />
	Para cualquier duda al respecto contactanos por correo a jacardenas@sahuayo.mx<br />
	Los formatos de alta serán geneados por cada Sucursal.</p>
...
// copia a correo de JRH
$IDmatriz = $row_matriz['IDmatriz'];
$query_jrh = "SELECT * FROM vac_usuarios WHERE IDusuario_puesto = 129 AND IDmatriz = '$IDmatriz'";
$jrh = mysql_query($query_jrh, $vacantes) or die(mysql_error());
$row_jrh = mysql_fetch_assoc($jrh);

do{ 
$mail->AddAddress($row_jrh['usuario']);
echo $row_jrh['usuario'];
} while($row_jrh = mysql_fetch_array($jrh));
