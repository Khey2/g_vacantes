...
{
echo "Archivo Cargado Con Exito";  header("Location: vd_importar.php?info=1"); 	
}
else
{
echo "Error Al Cargar el Archivo"; header("Location: vd_importar.php?info=2"); 
}
...
$IDempleado_s =		$objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue();
...
if (isset($_GET['mes'])) {$IDmes = $_GET['mes'];}
...
echo "Total elementos subidos: $totalIngresados "; header("Location: vd_importar.php?info=3"); 
}
					...
					<div class="row">
							<div class="panel panel-flat">
								<div class="panel-heading">
									<h6 class="panel-title">Ajuste de Comisiones VD</h6>
  						...
  						<?php
								mysql_select_db($database_vacantes, $vacantes);
								$query_puestos = "SELECT com_vd_temp.IDmes, com_vd_temp.fecha_antiguedad, com_vd_temp.calculado, com_vd_temp.Comisiones_pieza, com_vd_temp.Comisiones_caja, com_vd_temp.Premio_1, com_vd_temp.Premio_2, com_vd_temp.bt_01_fecha,  com_vd_temp.BonoVentaNeta, com_vd_temp.BonoClientesVenta, com_vd_temp.BonoDevPorc, com_vd_temp.bt_02_fecha, com_vd_temp.bt_03_fecha, com_vd_temp.bt_04_fecha, com_vd_temp.bt_05_fecha,com_vd_temp.IDvd, com_vd_temp.IDmatriz, com_vd_temp.IDempleadoS, com_vd_temp.IDempleado, com_vd_temp.Clave, com_vd_temp.VentaNeta, com_vd_temp.VentaNetaCajas, com_vd_temp.VentaNetaPieza, com_vd_temp.ClientesVenta, com_vd_temp.NoPedidos, com_vd_temp.Visitas, com_vd_temp.DevImporte, com_vd_temp.DevPorc, com_vd_temp.Presupuesto, com_vd_temp.Cubrimiento, com_vd_temp.MargenBruto, com_vd_temp.IDsemana, com_vd_temp.bt_01, com_vd_temp.bt_02, com_vd_temp.bt_03, com_vd_temp.bt_04, com_vd_temp.bt_05, com_vd_temp.bt_garantizado, com_vd_temp.bt_adicional,  com_vd_temp.bt_observaciones, com_vd_temp.bt_capturador, com_vd_temp.bt_fecha_captura, com_vd_temp.BonoProductividad, com_vd_temp.Premios, com_vd_temp.Comisiones, vac_matriz.matriz, Empleados.IDempleado, Empleados.emp_paterno AS emp_paterno, Empleados.emp_materno AS emp_materno, Empleados.emp_nombre AS emp_nombre, Empleados.denominacion AS emp_denominacion, Empleados.IDpuesto AS emp_IDpuesto, Jefes.IDempleado AS jefe_IDempleado, Jefes.emp_paterno AS jefe_paterno, Jefes.emp_materno AS jefe_materno, Jefes.emp_nombre AS jefe_nombre, Jefes.denominacion AS jefe_denominacion, Jefes.IDpuesto  AS jefe_IDpuesto FROM com_vd_temp LEFT JOIN prod_activos AS Empleados ON com_vd_temp.IDempleado = Empleados.IDempleado LEFT JOIN prod_activos AS Jefes ON com_vd_temp.IDempleadoS = Jefes.IDempleado LEFT JOIN vac_matriz ON com_vd_temp.IDmatriz = vac_matriz.IDmatriz";
								mysql_query("SET NAMES 'utf8'");
					...
					<table class="table table-condensed datatable-button-html5-columns">
                        ...
                        <?php do { $nombre = $row_puestos['emp_paterno']." ".$row_puestos['emp_materno']." ".$row_puestos['emp_nombre']; ?>
                          <tr>
                            ...
                            <td><?php if($row_puestos['IDempleado'] != 0) {echo $row_puestos['emp_denominacion']; } else {echo "VACANTE";} ?></td>
							...
							<p>&nbsp;</p>
								<ul class="media-list">
								<li><strong>Venta Cajas : </strong><?php echo "$" . number_format($row_puestos['Premio_1']); ?>&nbsp;</li>
								<li><strong>Venta Neta : </strong><?php echo "$" . number_format($row_puestos['Premio_2']); ?>&nbsp;</li>
								</ul>
							</div>
							</td>
							 <td>
