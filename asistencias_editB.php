...
if($_SESSION['IDsucursal'] == '' OR $_SESSION['IDsucursal'] == 0) {$X1 = '';}  else {$X1 = " prod_activosfaltas.IDsucursal = '$IDsucursal' AND "; }
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
 $insertSQL = sprintf("INSERT INTO ind_asistencia (IDempleado, emp_paterno, emp_materno, emp_nombre, denominacion, IDmatriz, IDpuesto, IDarea, IDvalidador, IDestatus, IDruta, anio, mes, IDfecha, IDtipov, comentarios) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
header("Location: asistencias_editB.php?anio=$el_anio&mes=$el_mes&IDfecha=$IDfecha&info=1");
}
...
$updateSQL = sprintf("UPDATE ind_asistencia SET IDestatus=%s, IDruta=%s, IDtipov=%s, IDvalidador=%s, comentarios=%s WHERE IDempleado = '$IDempleado' AND IDfecha = $IDfecha AND anio = $el_anio AND mes = $el_mes",
            GetSQLValueString($_POST['IDestatus'], "int"),
            GetSQLValueString($_POST['IDruta'], "int"),
            GetSQLValueString($_POST['IDtipo'], "int"),
            GetSQLValueString($_POST['IDvalidador'], "int"),
...
header("Location: asistencias_editB.php?anio=$el_anio&mes=$el_mes&IDfecha=$IDfecha&info=2");
}
            ...
            <?php } ?>
					...
					<div class="panel panel-flat">
						...
						<div class="panel-body">
							...
							a) Da clic en el botón Validar para realizar la validación la asistencia del colaborador con Ausentismo.<br/>
...
<?php  if ( $totalRows_sucursal > 1) {?>
					...
					<form method="POST" action="asistencias_editB.php?anio=<?php echo $el_anio?>&mes=<?php echo $el_mes?>&IDfecha=<?php echo $IDfecha?>">
					<table>
					<tr>
					<td>
					<div class="col-sm-12">
...
<?php } ?>
			...
			<table class="table table-condensed datatable-button-html5-columns">				
			 ...
			 <div onClick="loadDynamicContentModal('<?php echo $row_activos['IDempleado']; ?>', '<?php echo $IDfecha; ?>')" class="btn btn-xs btn-info btn-icon">Validar</div>
			 <?php } else if ($row_activos['IDcapturador'] == '' AND $row_activos['IDvalidador'] != '') { ?>
			 <div onClick="loadDynamicContentModal('<?php echo $row_activos['IDempleado']; ?>', '<?php echo $IDfecha; ?>')" class="btn btn-xs btn-primary btn-icon">Validado</div>
			 <?php } else if ($row_activos['IDcapturador'] > 0  AND $row_activos['IDvalidador'] == '') { ?>
			 <div onClick="loadDynamicContentModal('<?php echo $row_activos['IDempleado']; ?>', '<?php echo $IDfecha; ?>')" class="btn btn-xs btn-info btn-icon">Validar</div>
			 <?php } else if ($row_activos['IDcapturador'] > 0 AND $row_activos['IDvalidador'] > 0 ) { ?>
			 <div onClick="loadDynamicContentModal('<?php echo $row_activos['IDempleado']; ?>', '<?php echo $IDfecha; ?>')" class="btn btn-xs btn-primary btn-icon">Validado</div>	
			 <?php } ?>
					 </td>
                   ...
                   <td><?php  if ($row_activos['IDvalidador'] != ''){ 

					$IDtipov = $row_activos['IDtipov']; 
					$query_validacion = "SELECT * FROM ind_asistencia_tipos WHERE IDtipo = '$IDtipov'";
					$validacion = mysql_query($query_validacion, $vacantes) or die(mysql_error());
					$row_validacion = mysql_fetch_assoc($validacion);
					$totalRows_validacion = mysql_num_rows($validacion);

				   if ($totalRows_validacion > 0) { echo "<b>".$row_validacion['tipo']."</b>";} else { echo "Sin Ausencia"; } 
				   } else {echo "-";} ?></td>
                  </tr>
...
function loadDynamicContentModal(modal, IDfecha){
	...
	$('#conte-modal').load('asistencias_mdlB.php?mes=' + el_mes + '&anio=' + el_anio + '&IDfecha=' + IDfecha + '&IDempleado='+ modal, function() {
