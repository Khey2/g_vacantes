...
$noaplica = $_POST['noaplica'];  echo $noaplica;

  $updateSQL = "UPDATE sed_servicio SET IDrespuesta = '$actualiza_respuesta', noaplica = '$noaplica', observaciones = '$actualiza_observaciones' WHERE IDpregunta = '$la_pregunta_actual' AND IDempleado = '$el_usuario' AND anio = '$anio'"; 
  mysql_select_db($database_vacantes, $vacantes);
  $Result1 = mysql_query($updateSQL, $vacantes) or die(mysql_error());
  header("Location: servicio.php?IDpregunta=$la_pregunta_siguiente&activar=1");
}

//insertar
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {

$usuario_paterno = $row_usuario['usuario_parterno'];
$usuario_materno = $row_usuario['usuario_materno'];
$usuario_nombre = $row_usuario['usuario_nombre'];
$observaciones = $_POST['observaciones'];
$IDusuario_puesto = $row_usuario['IDusuario_puesto']; 
$noaplica = $_POST['noaplica']; echo $noaplica;

$insertSQL = sprintf("INSERT INTO sed_servicio (IDempleado, anio, fecha, IDpregunta, IDmatriz, IDrespuesta, usuario_paterno, usuario_materno, usuario_nombre, noaplica, observaciones, IDusuario_puesto) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($usuario_paterno, "text"),
                       GetSQLValueString($usuario_materno, "text"),
                       GetSQLValueString($usuario_nombre, "text"),
                       GetSQLValueString($noaplica, "int"),
                       GetSQLValueString($observaciones, "text"),
                       GetSQLValueString($IDusuario_puesto, "int"));
...
//cierre de la encuesta
	...
	$updateSQL = "INSERT INTO sed_servicio (IDempleado, anio, fecha, IDpregunta, IDrespuesta, IDmatriz) VALUES 
	('$el_usuario', '$anio', '$fecha', '$max_preg_extra', 9, '$IDmatriz')"; 
	mysql_select_db($database_vacantes, $vacantes);
	$Result1 = mysql_query($updateSQL, $vacantes) or die(mysql_error());
						...
						<div class="col-sm-6 col-md-2">
							<div class="panel-body">
								...
								<?php } elseif($totalRows_clima > 0) { // Si no termino la encuesta ?>
									...
									<a class="btn btn-warning btn-float btn-float-lg"  href="servicio.php?IDpregunta=<?php echo $la_pregunta_ultima + 1;?>&activar=1"><i class="icon-forward2"></i> <span>Continuar con la Encuesta</span> </a>
							
								<?php } else {  // Si no ha contestado nada ?>

									<a class="btn btn-info btn-float btn-float-lg" href="servicio.php?IDpregunta=<?php echo $la_pregunta_actual;?>&activar=1"><i class="icon-play3"></i> <span>Iniciar la Encuesta</span></a>
						 ...
						 <form method="post" id="form1" action="<?php echo $editFormAction; ?>" class="form-horizontal form-validate-jquery">
								...
								<fieldset class="content-group">
                                	...
                                	<div class="form-group">
										...
										<?php if ($IDarea == 11){ ?><span class="text text-semibold text-primary">Por favor indica el área o empleado del que consideras que debería mejorar su nivel de empatía.<span><?php } ?>
                                        </div>
                             ...
                             <?php if ($IDarea != 10){ ?>
								  ...
								  <div class="form-group">
                                        ...
                                        <div class="col-lg-6">
                                            ...
                                            <?php if ($la_respuesta != ""){ ?>
                                              <input type="range" min="50" max="100" value="<?php echo $row_respuesta['IDrespuesta']; ?>" step="5" class="form-control" id="IDrespuesta" name="IDrespuesta"> 
											<?php } else { ?>
											 <input type="range" min="50" max="100" value="50" step="5" class="form-control" id="IDrespuesta" name="IDrespuesta"> 
											<?php } ?>
										...
										<div class="checkbox">
											<label>
											<?php if ($row_respuesta['noaplica'] == 1){ ?>
												<input type="checkbox" class="control-danger" id="noaplica" name="noaplica" value="1" checked="checked">No aplica
											<?php } else { ?>
												<input type="checkbox" class="control-danger" id="noaplica" name="noaplica" value="1"><span class="text text-danger">No aplica</span>
											<?php } ?>
											</label>
										</div>
                             ...
                             <?php } ?>
								  ...
								  <div class="form-group">
										<div class="col-lg-12">
                                            <?php if ($la_respuesta != ""){ ?>
                                          <textarea name="observaciones" id="observaciones" rows="3" class="form-control" placeholder="Comenta aqui tus observaciones y sugerencias de mejora."><?php echo $row_respuesta['observaciones']; ?></textarea>
                                          	<?php } else { ?>
                                          <textarea name="observaciones" id="observaciones" rows="3" class="form-control" <?php if ($IDarea > 10){ ?> required="required" <?php } ?> placeholder="Comenta aqui tus observaciones y sugerencias de mejora."></textarea>
											<?php } ?>
					        ...
					        <input type="hidden" name="fecha" value="<?php echo $fecha; ?>" />
                                ...
                                <a class="btn bg-primary-300" href="servicio.php?IDpregunta=1&activar=1"><<- Primera</a>
                                <?php } ?>


								<?php  if ($la_pregunta_actual > 1) { ?> 
                                <a class="btn bg-primary-700" href="servicio.php?IDpregunta=<?php echo $la_pregunta_anterior; ?>&activar=1"><- Anterior</a>
                                <?php } ?>
                                ...
                                <a class="btn bg-primary-300" href="servicio.php?IDpregunta=<?php echo $la_pregunta_ultima + 1; ?>&activar=1">Última ->></a>
                                <?php } ?>
