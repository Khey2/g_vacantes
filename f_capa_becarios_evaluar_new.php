...
if ($totalRows_resultado == 0) {
	header("Location: f_capa_becarios.php?info=88"); 
}

$query_resultado2 = "SELECT * FROM capa_becarios_actividades WHERE IDempleado = $IDempleado AND IDmes = $el_mes AND anio = $anio AND estatus = 1";
...
if ($totalRows_resultado2 > 0 AND !isset($_GET['corte'])) {
	header("Location: f_capa_becarios.php?info=87"); 
}
...
$query1 = "UPDATE capa_becarios_actividades SET resultado = '$value', estatus = 1 WHERE IDempleado = $IDempleado AND IDmes = $el_mes AND anio = $anio AND IDactividad = $key";
$result1 = mysql_query($query1) or die(mysql_error());  	
  echo "Llave: '$key' Valor: '$value'";
}
}
header("Location: f_capa_becarios.php?info=99"); 
}
	...
	<div class="media panel-body no-margin">
		<div class="media-body">
				...
				<form method="post" name="form1" action="<?php echo $editFormAction; ?>">
					...
					<p>Las actividades que evaluarás a continuación fueron previamente capturadas por tu aprendiz/estudiante. Califícalas según su desempeño mensual; te recomendamos realizar una retroalimentación verbal de las mismas.</p>
					<p>Escala de evaluación:
					<ul>
					<li><b>Sobresaliente:</b> Superó las expectativas.</li>
					<li><b>Satisfactorio:</b> Logró los resultados esperados.</li>
					<li><b>Suficiente:</b> Ejecutó la actividad con supervisión continua.</li>
					<li><b>Deficiente:</b> No realizó la tarea enlistada.</li>
					</ul>
					</p>
					<p>Importante: Al evaluarlas estarás validando que las haya realizado.</p>
