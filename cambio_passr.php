...
$query_usuario = "SELECT vac_key.IDlogin, vac_key.llave, vac_usuarios.usuario FROM vac_usuarios LEFT JOIN vac_key ON vac_usuarios.usuario = vac_key.usuario WHERE vac_usuarios.usuario = '$elusuario' ORDER BY vac_key.IDlogin DESC LIMIT 1";
...
if ((isset($_GET['key'])) && ($_GET['key'] != $llave)) { $error = 1; }  else {
  ...
  $updateSQL = sprintf("UPDATE vac_usuarios SET password=%s WHERE usuario= '$elusuario'",
