...
elseif (isset($_SESSION['el_anio'])) {$el_anio = $_SESSION['el_anio'];} 
else {$el_anio = $anio;}
$_SESSION['el_anio'] = $el_anio;

if(isset($_POST['el_area'])) {$el_area = $_POST['el_area'];} 
elseif (isset($_SESSION['el_area'])) {$el_area = $_SESSION['el_area'];} 
...
mysql_select_db($database_vacantes, $vacantes);
$query_tareas = "SELECT ztar_tareas.IDtarea, ztar_tareas.anio, ztar_tareas.IDarea_rh, ztar_tareas.descripcion,  ztar_tareas.ponderacion, ztar_periodicidad.periodicidad, ztar_areas_rh.area_rh, Count(ztar_avances.IDavance), ztar_avances.fecha_esperada, ztar_valor_areas.valor_area FROM ztar_tareas LEFT JOIN ztar_periodicidad ON ztar_periodicidad.IDperiodicidad = ztar_tareas.IDperiodicidad LEFT JOIN ztar_areas_rh ON ztar_areas_rh.IDarea_rh = ztar_tareas.IDarea_rh LEFT JOIN ztar_avances ON ztar_avances.IDtarea = ztar_tareas.IDtarea LEFT JOIN ztar_valor_areas ON ztar_valor_areas.IDarea_rh = ztar_areas_rh.IDarea_rh WHERE ztar_tareas.anio = '$el_anio' AND ztar_valor_areas.anio = '$el_anio' GROUP BY ztar_tareas.IDtarea, ztar_tareas.IDarea_rh, ztar_tareas.descripcion, ztar_tareas.ponderacion, ztar_periodicidad.periodicidad, ztar_areas_rh.area_rh"; 
mysql_query("SET NAMES 'utf8'");
...
mysql_select_db($database_vacantes, $vacantes);
$query_arrhl = "SELECT * FROM ztar_areas_rh WHERE IDarea_rh = '$el_area'";
			...
			<?php require_once('assets/pheader.php'); ?>
				...
				<div class="content">
					    ...
					    <!-- /basic alert -->
							...
							<p>A continuación se muestran los objetivos de desempeño para los Jefes de Rh en Sucursales para el <?php echo $el_anio ?>.</p>
                             
					<form method="POST" action="objetivos_a.php">
					<table class="table">
						<tbody>							  
							<tr>
							<td>
						...
						<?php do { $IDtarea = $row_tareas['IDtarea'];?>
								<tr>
					                ...
					                <td><div class="text-semibold"><?php echo $row_tareas['area_rh']. " (" . $row_tareas['valor_area'] ."%)"; ?></div></td>
						...
						<?php } else {?>
                        <td colspan="9">No tiene tareas asignadas para ese año.</td>
						<?php } ?>
							</tbody>
						</table>
					</div>
