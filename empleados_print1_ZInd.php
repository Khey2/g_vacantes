...
mysql_select_db($database_vacantes, $vacantes);
$query_contratos = "SELECT con_empleados.*, con_bancos.banco, vac_matriz.matriz, vac_matriz.matriz_cv, vac_matriz.direccion, vac_empresas.empresa,  vac_matriz.firma_contratos, vac_empresas.rep_legal AS Rep_legal, vac_empresas.direccion AS Direccion FROM con_empleados LEFT JOIN vac_matriz ON  con_empleados.IDmatriz = vac_matriz.IDmatriz LEFT JOIN vac_puestos ON  con_empleados.IDpuesto = vac_puestos.IDpuesto LEFT JOIN con_bancos ON  con_empleados.a_banco = con_bancos.IDbanco LEFT JOIN vac_empresas ON  con_empleados.IDempresa = vac_empresas.IDempresa WHERE con_empleados.IDempleado = $IDempleado";
...
$empresa = $row_contratos['empresa'];
$rep_legal = $row_contratos['Rep_legal'];
...
<p class="justificar"> En '.$ubicacionfirma.', a <strong>'.$ini_cont_d.' de '.$ini_cont_m.' de '.$ini_cont_y.'</strong>, los que suscribimos el presente contrato a saber <strong>'.$empresa.'</strong> Representada por <strong>'.$rep_legal.'</strong>, por la otra <strong>'.$el_empleado.'</strong> por sus propios derechos con domicilios respectivos en <strong>'.$direccion_emdireccion_empresapresa.'</strong> y en <strong>Calle '.$row_contratos['d_calle'].', '.$row_contratos['d_numero_calle'].', Colonia '.$row_contratos['d_colonia'].', C.P. '.$row_contratos['d_codigo_postal'].', '.$row_contratos['d_delegacion_municipio'].', '.$row_contratos['d_estado'].',</strong> hacemos constar que hemos convenido celebrar un contrato individual de  trabajo al tenor de las siguientes:</p>
...
<p class="justificar"><strong>Vigésima Cuarta.-</strong> El trabajador está obligado a informar y entregar a la empresa cualquier documento emitido y proporcionado por el Instituto Mexicano del Seguro Social con todo lo relacionado a Riesgos Trabajo e Incapacidades. </p>

<p class="justificar"><strong>Vigésima Quinta.-</strong> Las partes convienen en que todo lo que no previsto en el presente contrato se regirá por lo dispuesto en la Ley Federal del Trabajo y que para la interpretación y cumplimiento se someten expresamente a la jurisdicción y competencia de la Autoridad Laboral Competente en turno y de la Entidad Federativa que corresponda al domicilio de su centro de trabajo.</p> 
