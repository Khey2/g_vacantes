...
$fecha3a = $_POST['fecha_cierre']; 
if ($fecha3a == ""){$fecha3 = '';} else {
$fecha3b = explode("-",$fecha3a);
$fecha3 = $fecha3b[2]."-".$fecha3b[1]."-".$fecha3b[0];
}
...
$val4 = $_POST['val4']; 
  ...
  $updateSQL = sprintf("UPDATE pp_prueba SET IDpuesto_destino=%s, IDmatriz_destino=%s, sueldo_actual=%s, IDarea_destino=%s,  fecha_inicio=%s, fecha_fin=%s, fecha_cierre=%s, observaciones=%s, sueldo_nuevo=%s, IDestatus=%s, val1=%s, val2=%s, val3=%s, val4=%s WHERE IDpprueba=%s",
...
header("Location: admin_pprueba_edit_2.php?IDpprueba=$IDpprueba&info=1");
}
...
if ((isset($_GET['IDpprueba_borrar'])) && ($_GET['IDpprueba_borrar'] != "")) {
  ...
  header("Location: admin_pprueba.php?info=3");
}
...
<!-- Page container -->
		...
		<?php require_once('assets/menu.php'); ?>
			...
			<?php require_once('assets/pheader.php'); ?>
				...
				<div class="content">
					    ...
					    <div class="alert bg-warning-600 alert-styled-left">
							<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Cerrar</span></button>
							El periodo de prueba debe ser mayor a 3 meses y menor a 6.
					    </div>
                        <?php } ?>
					    <!-- /basic alert -->
                  ...
                  <div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Agregar Periodo de Prueba</h5>
						</div>
	...
	<legend class="text-semibold">Puesto destino</legend>
									...
									<div class="form-group">
										...
										<div class="col-lg-9">
											<select name="IDpuesto_destino" id="IDpuesto_destino" class="bootstrap-select" data-live-search="true" onchange="realizaProceso($('#IDpuesto_destino').val(), $('#IDmatriz_destino').val());return false;"data-width="100%" required="required">
												<option value="">Seleccione una opción</option> 
												  <?php  do { ?>
				...
				<legend class="text-semibold">Fechas</legend>
                                    ...
                                    <span class="input-group-addon"><i class="icon-calendar22"></i></span>
                                    	<input type="text" class="form-control  pickadate-format" name="fecha_inicio" id="fecha_inicio" value="<?php if ($row_pprueba['fecha_inicio'] == "") { echo "";} else { echo date('d-m-Y', strtotime($row_pprueba['fecha_inicio'])); }?>"  required="required">
									</div>
                                   </div>
                                  </div> 
									<!-- Fecha -->
			                        ...
			                        <div class="input-group">
                                    <span class="input-group-addon"><i class="icon-calendar22"></i></span>
                                    	<input type="text" class="form-control  pickadate-format" name="fecha_fin" id="fecha_fin" value="<?php if ($row_pprueba['fecha_fin'] == "") { echo "";} else { echo date('d-m-Y', strtotime($row_pprueba['fecha_fin'])); }?>"  required="required">
									</div>
                                   </div>
                                  </div> 
									<!-- Fecha -->
									...
									<!-- Fecha -->
                                    <div class="form-group">
										<label class="control-label col-lg-3">Fecha entrega a Nómina:<span class="text-danger">*</span></label>
			                        <div class="col-lg-9">
			                        <div class="input-group">
                                    <span class="input-group-addon"><i class="icon-calendar22"></i></span>
                                    	<input type="text" class="form-control pickadate-format" placeholder="Capturar fecha" name="fecha_cierre" id="fecha_cierre" value="<?php if ($row_pprueba['fecha_cierre'] == "") { echo "";} else { echo date('d-m-Y', strtotime($row_pprueba['fecha_cierre'])); }?>">
									</div>
								   </div>
                                  </div> 
									<!-- Fecha -->
	...
	<legend class="text-semibold">Validación de Política</legend>
									...
									<div class="form-group">
										...
										<div class="col-lg-9">
											<select name="IDestatus" id="IDestatus" class="form-control"  required="required">
												  <option value="1"<?php if ($row_pprueba['IDestatus'] == 1) {echo "SELECTED";} ?>>Captura Incompleta</option>
												  <option value="2"<?php if ($row_pprueba['IDestatus'] == 2) {echo "SELECTED";} ?>>Cargar Desempeño</option>
												  <option value="3"<?php if ($row_pprueba['IDestatus'] == 3) {echo "SELECTED";} ?>>Capturar Pagos</option>
												  <option value="4"<?php if ($row_pprueba['IDestatus'] == 4) {echo "SELECTED";} ?>>Cargar Notifiación Firmada</option>
												  <option value="5"<?php if ($row_pprueba['IDestatus'] == 5) {echo "SELECTED";} ?>>Imprimir Afectacion</option>
												  <option value="6"<?php if ($row_pprueba['IDestatus'] == 6) {echo "SELECTED";} ?>>Terminado</option>
											</select>
										</div>
									</div>
									<!-- /basic text input -->
                    	 ...
                    	 <button type="button" onClick="window.location.href='admin_pprueba.php'" class="btn btn-default btn-icon">Regresar</button>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body">
									<p>¿Estas seguro que quieres borrar el Periodo de Prueba?.</p>
								</div>

								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                                    <a class="btn btn-danger" href="admin_pprueba_edit_2.php?IDpprueba_borrar=<?php echo $IDpprueba; ?>">Si borrar</a>
								</div>
							</div>
						</div>
					</div>
