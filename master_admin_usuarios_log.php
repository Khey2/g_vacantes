...
mysql_select_db($database_vacantes, $vacantes);
$query_usuarios = "SELECT vac_login_stats.id, vac_usuarios.usuario, vac_usuarios.IDusuario, vac_usuarios.usuario_nombre, vac_usuarios.usuario_parterno, vac_usuarios.usuario_materno, MAX(vac_login_stats.last_logindate_log) AS lahora, vac_matriz.matriz, vac_matriz.IDmatriz FROM vac_login_stats LEFT JOIN vac_usuarios ON vac_usuarios.IDusuario = vac_login_stats.id_log INNER JOIN vac_matriz ON vac_matriz.IDmatriz = vac_usuarios.IDmatriz WHERE vac_login_stats.last_logindate_log >= DATE_SUB(NOW(), INTERVAL 48 HOUR) GROUP BY vac_usuarios.usuario ORDER BY vac_login_stats.last_logindate_log ASC";
