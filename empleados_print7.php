...
mysql_select_db($database_vacantes, $vacantes);
$query_contratos = "SELECT con_empleados.*, con_bancos.banco, vac_matriz.matriz, vac_matriz.matriz_cv, vac_matriz.direccion, vac_puestos.denominacion FROM con_empleados LEFT JOIN vac_matriz ON con_empleados.IDmatriz = vac_matriz.IDmatriz LEFT JOIN vac_puestos ON con_empleados.IDpuesto = vac_puestos.IDpuesto LEFT JOIN con_bancos ON con_empleados.a_banco = con_bancos.IDbanco WHERE con_empleados.IDempleado = $IDempleado";
mysql_query("SET NAMES 'utf8'");
...
if ($tipo_nomina == 1){$tipo_nomina = "semanalmente";} else {$tipo_nomina = "quincenalmente";}
...
<p  class="justificar">Por este medio, hacemos constar que el C. <strong>'.$row_contratos['a_nombre'].' '.$row_contratos['a_paterno'].' '.$row_contratos['a_materno'].'</strong>, es consciente de su n&#250;mero de cuenta de banco (Santander), donde '.$tipo_nomina.' se le estar&#225; depositando.</p>
&nbsp; <br>

<p>N&#250;mero de cuenta: <strong>'.$cuenta_bancaria.'</strong></p>
<p>Banco: <strong>'.$elbanco.'</strong></p>
<p>Leyenda: _____________________________________________________</p>
&nbsp; <br>

<p>Se extiende la presente para los fines que crea conveniente darles el C. '.$row_contratos['a_nombre'].' '.$row_contratos['a_paterno'].' '.$row_contratos['a_materno'].'.</p>
&nbsp; <br>
&nbsp; <br>
...
$nombreDelDocumento = "CUENTA DE BANCO ".date('dmY')." ".$IDempleado.".pdf";
