...
$IDtipo = $_GET['IDtipo'];
mysql_select_db($database_vacantes, $vacantes);
$query_resultado = "SELECT * FROM proced_tipos WHERE proced_tipos.IDtipo = '$IDtipo'";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	$updateSQL = sprintf("UPDATE proced_tipos SET tipo=%s WHERE IDtipo=%s",
  ...
  $updateGoTo = "admin_proced_tipos.php?info=2";
  if (isset($_SERVER['QUERY_STRING'])) {
  ...
  $insertSQL = sprintf("INSERT INTO proced_tipos (tipo) VALUES (%s)",
                       GetSQLValueString($_POST['tipo'], "text"));

  mysql_select_db($database_vacantes, $vacantes);
  $Result1 = mysql_query($insertSQL, $vacantes) or die(mysql_error());

  $insertGoTo = "admin_proced_tipos.php?info=1";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_GET['borrar'])) && ($_GET['borrar'] != "")) {
  ...
  $borrado = $_GET['IDtipo'];
  $IDtipo = $_GET['IDtipo'];
  $deleteSQL = "DELETE FROM proced_tipos WHERE IDtipo ='$borrado'";

  mysql_select_db($database_vacantes, $vacantes);
  $result = mysql_query($deleteSQL, $vacantes) or die(mysql_error());
  header("Location: admin_proced_tipos.php?info=3");
}
...
<div class="content">
                  ...
                  <div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Editar / Actualizar Tipo de Documento</h5>
						</div>

					<div class="panel-body">
									<?php if (isset($_GET['IDtipo'])) {?>
                            ...
                            <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="form-horizontal form-validate-jquery">
                                   ...
                                   <!-- Basic text input -->
									<div class="form-group">
										...
										<div class="col-lg-9">
						<input type="text" name="tipo" id="tipo" class="form-control" value="<?php echo htmlentities($row_resultado['tipo'], ENT_COMPAT, ''); ?>" required="required">
										</div>
									</div>
									<!-- /basic text input -->
                                    
                         <input class="btn bg-success btn-icon" type="submit" value="Actualizar" />
                         <button type="button" onClick="window.location.href='admin_proced_tipos.php'" class="btn btn-default btn-icon">Regresar</button>
                           ...
                           <p>Agregar la información solicitada.</p>
                            ...
                            <form method="post" name="form1" action="<?php echo $editFormAction; ?>" class="form-horizontal form-validate-jquery">
									...
									<div class="form-group">
										<label class="control-label col-lg-3">Tipo:</label>
										<div class="col-lg-9">
                        <input type="text" name="tipo" id="tipo" class="form-control" value="" required="required">
										</div>
									</div>
									<!-- /basic text input -->
