...
if (isset($_GET['IDdocumento'])) {$IDdocumento_filtro = " AND proced_documentos.IDdocumento = ".$_GET['IDdocumento'];} else {$IDdocumento_filtro = '';}

mysql_select_db($database_vacantes, $vacantes);
$query_documentos = "SELECT proced_documentos.IDdocumento, proced_documentos.IDDarea, proced_documentos.vistas, proced_documentos.IDdireccion, proced_documentos.IDsubarea, proced_documentos.IDtipo, proced_documentos.IDvisible, proced_documentos.documento, proced_documentos.descripcion, proced_documentos.file, proced_documentos.version, proced_documentos.anio, proced_documentos.vistas, proced_subareas.subarea, proced_direcciones.direccion, proced_areas.area, proced_tipos.tipo, proced_puestos.IDpuesto FROM proced_documentos	LEFT JOIN proced_puestos ON proced_documentos.IDdocumento = proced_puestos.IDdocumento LEFT JOIN proced_subareas ON proced_documentos.IDsubarea = proced_subareas.IDsubarea LEFT JOIN  proced_direcciones ON proced_documentos.IDdireccion = proced_direcciones.IDdireccion LEFT JOIN proced_areas ON proced_documentos.IDDarea = proced_areas.IDDarea LEFT JOIN proced_tipos ON proced_documentos.IDtipo = proced_tipos.IDtipo WHERE proced_puestos.IDpuesto = $IDpuesto AND proced_documentos.IDvisible = $IDvisible".$IDdocumento_filtro.$filtroBuscado.$filtroIDdireccion.$filtroIDDarea.$filtroIDsubarea." ORDER BY proced_documentos.anio desc";
...
mysql_select_db($database_vacantes, $vacantes);
$query_doc_totales = "SELECT proced_documentos.*, proced_puestos.IDpuesto FROM proced_documentos LEFT JOIN proced_puestos ON proced_documentos.IDdocumento = proced_puestos.IDdocumento WHERE IDvisible = 1 AND proced_puestos.IDpuesto = $IDpuesto";
...
<body class="has-detached-right <?php if (isset($_COOKIE["lmenu"])) { echo 'sidebar-xs';}?>">
            ...
            <?php  } ?>
				...
				<div class="panel panel-flat">
					...
					<div class="panel panel-flat">
						...
						<div class="panel-body">
							<form action="f_procedimientos.php" method="post" class="main-search">
					...
					<!-- /search field -->
                        ...
                        $resultado_file1 = strtolower($valores1[count($valores1)-1]);
                        ?>
							<!-- List -->
							<ul class="media-list">
                                ...
                                <div class="media-left">
									...
									<div class="media-body">
										...
										<ul class="list-inline list-inline-separate">
											...
											<?php if ($row_documentos['vistas'] > 0) { ?>
											<li><strong>Vistas: </strong> <a href="#"><?php echo $row_documentos['vistas']; ?></a></li>
											<?php } ?>
										</ul>
									</div>
									...
									<?php if ($resultado_file1 == 'pdf')  { ?>
									<div onClick="loadDynamicContentModal('<?php echo $row_documentos['IDdocumento']; ?>')" class="btn bg-primary-400 mt-15 btn-block"><i class="icon-file-eye2"></i> Visualizar</div>
									<?php } else { ?>
									<a href="f_procedimientos_file.php?IDdocumento=<?php echo $row_documentos['IDdocumento'] ?>" class="btn bg-success-400 mt-15 btn-block"><i class="icon-file-download2 position-left"></i>Descargar</a>
									<?php } ?>


									<?php if ( $descargar == 1)  { ?>
									<a href="f_procedimientos_file.php?IDdocumento=<?php echo $row_documentos['IDdocumento'] ?>" class="btn bg-success-400 mt-15 btn-block" target="_blank"><i class="icon-file-download2 position-left"></i>Descargar</a>
									<?php } ?>
	...
	<div class="sidebar-category">
		...
		$query_direcciones_count = "SELECT proced_documentos.*, proced_puestos.IDpuesto FROM proced_documentos LEFT JOIN proced_puestos ON proced_documentos.IDdocumento = proced_puestos.IDdocumento WHERE proced_puestos.IDpuesto = $IDpuesto AND IDdireccion = $la_direccion AND IDDarea = '' AND IDsubarea = '' AND IDvisible = 1";
		...
		$totalRows_direcciones_count = mysql_num_rows($direcciones_count);
				?>
						...
						<li style="list-style: none;">
							<strong><a style="color:#352C2C;" href="f_procedimientos.php?IDdireccion=<?php echo $row_direcciones['IDdireccion'] ?>"><i class="icon-forward3"></i> <?php echo $row_direcciones['direccion'] ?></a> (<?php echo $totalRows_direcciones_count; ?>)</strong>
		...
		$query_areas_count = "SELECT proced_documentos.*, proced_puestos.IDpuesto FROM proced_documentos LEFT JOIN proced_puestos ON proced_documentos.IDdocumento = proced_puestos.IDdocumento WHERE proced_puestos.IDpuesto = $IDpuesto AND IDdireccion = $la_direccion AND IDDarea = $la_area AND IDsubarea = ''  AND IDvisible = 1";
		...
		$totalRows_areas_count = mysql_num_rows($areas_count);
					...
					<a href="f_procedimientos.php?IDdireccion=<?php echo $row_direcciones['IDdireccion'] ?>&IDDarea=<?php echo $row_areas['IDDarea'] ?>"><i class="icon-arrow-right5"></i> <?php echo $row_areas['area'] ?> (<?php echo $totalRows_areas_count; ?>)</a>
					<?php if ($row_areas['IDDarea'] == 93) { echo "</strong>"; } ?>					
		...
		$query_subareas_count = "SELECT proced_documentos.*, proced_puestos.IDpuesto FROM proced_documentos LEFT JOIN proced_puestos ON proced_documentos.IDdocumento = proced_puestos.IDdocumento WHERE proced_puestos.IDpuesto = $IDpuesto AND IDdireccion = $la_direccion AND IDDarea = $la_area AND IDsubarea = $la_subarea  AND IDvisible = 1";
		...
		$totalRows_subareas_count = mysql_num_rows($subareas_count);
						?>
									...
									<li style="list-style: none;"><i class="icon-arrow-right22"></i> <a href="f_procedimientos.php?IDdireccion=<?php echo $row_direcciones['IDdireccion'] ?>&IDDarea=<?php echo $row_areas['IDDarea'] ?>&IDsubarea=<?php echo $row_subarea['IDsubarea'] ?>"><?php echo $row_subarea['subarea'] ?> (<?php echo $totalRows_subareas_count; ?>)</a></li>
...
function loadDynamicContentModal(modal){
	...
	$('#conte-modal').load('f_procedimientos_mdl.php?IDdocumento='+ modal, function() {
