...
//evaluadas
$query_mis_evaluadas = "SELECT Count(sed_individuales.IDmeta) AS Total FROM sed_individuales WHERE sed_individuales.IDempleado = '$el_usuario' AND sed_individuales.IDperiodo = '$IDperiodo' AND sed_individuales.estatus = 2"; 
...
//evaluadas por jefe
$query_mis_evaluadas_boss = "SELECT Count(sed_individuales.IDmeta) AS Total FROM sed_individuales WHERE sed_individuales.IDempleado = '$el_usuario' AND sed_individuales.IDperiodo = '$IDperiodo' AND sed_individuales.estatus = 3"; 
$mis_evaluadas_boss = mysql_query($query_mis_evaluadas_boss, $vacantes) or die(mysql_error());
$row_mis_evaluadas_boss = mysql_fetch_assoc($mis_evaluadas_boss);
$totalRows_mis_evaluadas_boss = mysql_num_rows($mis_evaluadas_boss);
$metas_evaluadas_boss = $row_mis_evaluadas_boss['Total'];
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $mi_obs = addslashes($_POST['mi_obs']);	
  //str_replace("'", "", $$mi_obs);
  ...
  header("Location: f_desemp_proponer.php?info=1&IDperiodo=$IDperiodo&IDempleado=$el_usuario&id=$id");
...
// ya hay resultados cargados
  ...
  header("Location:  f_desemp_proponer.php?info=$terminacaptura&IDperiodo=$IDperiodo&IDempleado=$IDempleado&id=$id");
}
...
<head>
	...
	<script>
      ...
      function load() {
       new Noty({
            text: 'Tiene '+metas_pendientes+' objetivos sin proponer.',
            type: 'warning'
        }).show();
    }
	 window.onload = load;
	...
	<?php if (($metas_evaluadas == $metas_capturadas) && $estatus_actual == 1) { ?> 
	<script>
      function load() {
       new Noty({
	...
	<?php if ($estatus_actual == 2) { ?> 
	<script>
      function load() {
       new Noty({
            text: 'La propuesta de evaluación ha terminado, gracias por tu participación.',
            type: 'success'
        }).show();
    }
	 window.onload = load;
							...
							<div class="panel panel-flat">
								...
								<div class="media panel-body no-margin">
									<div class="media-body">
                                      <ol>
        <li>Asegurate de proponer el resulado todos los objetivos.</li>
        <li>Da clic en el botón Proponer Resultado o Actualizar, según corresponda.</li>
        <li><strong>Una vez que hayas terminado de evaluar, da clic en el botón Terminar Evaluación.</strong></li>
        <li>Tus propuestas serán revisadas por tu Jefe Inmediato.</li>
                                      </ol>
                                    </div>
								</div>
							</div>
					...
					<?php $count = 1;  do { $IDmeta = $row_mis_metas['IDmeta']; ?>
							...
							<div class="panel panel-white">
								<div class="panel-heading">
									...
									<?php if ($row_mis_metas['estatus'] == 2) {echo "<span class='label label-warning'>Resultado Propuesto</span>";} ?>
									...
									<div class="heading-elements">
                                        ...
                                        <li>
											<?php if ($row_mis_metas['estatus'] == 2 && $estatus_actual == 1) {?>
                          <button type="button" data-target="#actualizar<?php echo $row_mis_metas['IDmeta']; ?>" data-toggle="modal" class="btn btn-warning btn-xs">Actualizar</button>
											<?php } elseif ($row_mis_metas['estatus'] == 1 or $row_mis_metas['estatus'] == 0) {?>
                          <button type="button" data-target="#actualizar<?php echo $row_mis_metas['IDmeta']; ?>" data-toggle="modal" class="btn btn-warning btn-xs">Proponer Resultado</button>
											<?php } else {?>
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content text-center">
								...
								<button type="button" class="close" data-dismiss="modal">&times;</button>
               					  <h5 class="modal-title">Proponer Resultados - Objetivo de Desempeño</h5>
								</div>
            					...
            					<form method="post" class="form-horizontal form-validate-jquery" name="form1" action="f_desemp_proponer.php?IDperiodo=<?php echo $IDperiodo; ?>&IDempleado=<?php echo $IDempleado; ?>&id=<?php echo $_GET['id']; ?>" > 
								  ...
								  <div class="form-group">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
											...
											<?php if ($row_mis_metas['estatus'] == 2 && $estatus_actual == 1) {?>
                           <input type="submit" class="btn btn-warning" value="Proponer Resultado">
											<?php } elseif ($row_mis_metas['estatus'] == 1 or $row_mis_metas['estatus'] == 0) {?>
                           <input type="submit" class="btn btn-warning" value="Proponer Resultado">
											<?php } else {?>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
                                    ...
                                    <a class="btn btn-warning" href="f_desemp_proponer.php?IDperiodo=<?php echo $IDperiodo; ?>&IDempleado=<?php echo $el_usuario;?>&id=<?php echo $_GET['id']; ?>&terminacaptura=0">Si Cerrar</a>
								</div>
							</div>
						</div>
					</div>
					...
					<div id="abrir" class="modal fade" tabindex="-1">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header bg-warning">
									<button type="button" class="close" data-dismiss="modal">&times;</button>
									<h6 class="modal-title">Abrir Evaluación</h6>
								</div>
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Regresar</button>
                                    <a class="btn btn-warning" href="f_desemp_proponer.php?IDperiodo=<?php echo $IDperiodo; ?>&IDempleado=<?php echo $el_usuario;?>&id=<?php echo $_GET['id']; ?>&terminacaptura=1">Si Habilitar</a>
								</div>
							</div>
						</div>
					</div>
							...
							<div class="sidebar-content">
								...
								<div class="sidebar-category">
									...
									<div class="category-content">
                                        ...
                                        <?php if (($metas_capturadas == $metas_propuestas) && ($estatus_actual == 1)) { ?>
	                                 <p><button type="button" data-target="#cierre" data-toggle="modal" class="btn btn-xs btn-success btn-block content-group">Terminar Evaluación</button></p>
                                        <?php } ?> 
                                         ...
                                         <?php if ($estatus_actual == 2 && $metas_evaluadas_boss == 0) { // abrir de nuevo ?>
									...
									echo '<span class="control-label no-margin text-success">' . $ponderacion_total . '% (Correcto)</span>'; } ?>
                                     ...
                                     <p><strong>Objetivos Propuestos:</strong>
                                      <?php if ($metas_evaluadas == $metas_capturadas) { 
