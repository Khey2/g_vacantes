...
mysql_select_db($database_vacantes, $vacantes);
$query_matriz = "SELECT vac_matriz.matriz, vac_vacante.IDvacante, vac_vacante.ajuste_dias, vac_vacante.observaciones, vac_puestos.dias, vac_areas.area, vac_vacante.IDestatus, vac_vacante.fecha_requi, vac_matriz.IDmatriz, vac_puestos.denominacion, vac_apoyo.apoyo FROM vac_vacante LEFT JOIN vac_areas ON vac_areas.IDarea = vac_vacante.IDarea LEFT JOIN vac_matriz ON vac_matriz.IDmatriz = vac_vacante.IDmatriz LEFT JOIN vac_puestos ON vac_puestos.IDarea = vac_areas.IDarea AND vac_vacante.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_apoyo ON vac_apoyo.IDapoyo = vac_vacante.IDapoyo WHERE vac_vacante.IDestatus = 1 AND vac_matriz.IDmatriz IN (4,7,10,25,30)";
$matriz = mysql_query($query_matriz, $vacantes) or die(mysql_error());
$totalRows_matriz = mysql_num_rows($matriz);

if ($totalRows_matriz > 0) { 
...
while($row_matriz=mysql_fetch_array($matriz)){
 ...
 $startdate = date('Y/m/d', strtotime($row_matriz['fecha_requi']));
 $end_date =  date('Y/m/d');
 $previo = getWorkingDays($startdate, $end_date, $holidays);
 $ajuste_dias = $row_matriz['ajuste_dias'];
