...
$ins_vac_vacante->registerTrigger("END", "Trigger_Default_Redirect", 99, "vacantes_activas.php?info=1");
...
if (isset($row_vacante['IDrequi']) and $row_vacante['IDrequi'] == 1){
...
if ((isset($_GET['IDvacante_borrar'])) && ($_GET['IDvacante_borrar'] != "")) {
  ...
  header("Location: vacantes_activas.php?info=3");
}
...
if($IDmatriz != 7){
mysql_select_db($database_vacantes, $vacantes);
$query_puesto = "SELECT DISTINCT prod_plantilla.IDplantilla, prod_plantilla.IDpuesto, vac_puestos.denominacion, vac_puestos.IDarea  FROM prod_plantilla INNER JOIN vac_puestos ON prod_plantilla.IDpuesto = vac_puestos.IDpuesto WHERE prod_plantilla.IDmatriz = $IDmatriz GROUP BY prod_plantilla.IDpuesto ORDER BY vac_puestos.denominacion ASC";
$puesto = mysql_query($query_puesto, $vacantes) or die(mysql_error());
$row_puesto = mysql_fetch_assoc($puesto);
$totalRows_puesto = mysql_num_rows($puesto);
} else {
mysql_select_db($database_vacantes, $vacantes);
$query_puesto = "SELECT DISTINCT vac_puestos.denominacion, vac_puestos.IDarea, vac_puestos.IDpuesto  FROM vac_puestos GROUP BY vac_puestos.IDpuesto ORDER BY vac_puestos.denominacion ASC";
$puesto = mysql_query($query_puesto, $vacantes) or die(mysql_error());
$row_puesto = mysql_fetch_assoc($puesto);
$totalRows_puesto = mysql_num_rows($puesto);	
}
				...
				<div class="content">
                  ...
                  <div class="panel panel-flat">
					...
					<p>Ingresa la información solicitada. Los campos marcados con <span class="text-danger">*</span> son obligatorios.</br>
  					...
  					Asegurate de actualizar el estatus de la vacante cuando se haya cubierto.</br>
					...
					<p><strong>Si no se tiene el foramto de Requisición autorizada, indicar la fecha de solicitud de la misma.</strong></p>
                    <?php  if (@$_GET['IDvacante'] != "") { ?>
					<p><strong>El campos de Fecha Requi o de Solicitud (si no hay Requi autorizada), solo puede modificarse si la vacante no tiene Requisición.</strong></p>
                    <?php } ?>
                  <p>&nbsp;</p>
                    <div>
                    <div>
    <form method="post" id="form1" action="<?php echo KT_escapeAttribute(KT_getFullUri()); ?>" enctype="multipart/form-data" class="form-horizontal form-validate-jquery">
                    ...
                    <?php $cnt1++; ?>
							...
							<!-- Fecha -->
			                        ...
			                        <div class="input-group">
                                    <span class="input-group-addon"><i class="icon-calendar22"></i></span>
                                    	<input type="text" class="form-control  daterange-single" required="required" name="fecha_ocupacion_<?php echo $cnt1; ?>" id="fecha_ocupacion_<?php echo $cnt1; ?>" value="<?php if ($row_rsvac_vacante['fecha_ocupacion'] == "") { echo "";} else  { echo KT_formatDate($row_rsvac_vacante['fecha_ocupacion']); }?>">
									</div>
                                   </div>
                                  </div> 
									<!-- Fecha -->
									...
									<!-- Basic select -->
									<div class="form-group">
										<label class="control-label col-lg-3">Motivo/Tipo de Vacante:<span class="text-danger">*</span></label>
										<div class="col-lg-9">
						...
						<input type="number" name="ajuste_dias_<?php echo $cnt1; ?>" id="ajuste_dias_<?php echo $cnt1; ?>" class="form-control" placeholder="Indica los días de ajuste." value="<?php echo KT_escapeAttribute($row_rsvac_vacante['ajuste_dias']); ?>">
									...
									<div class="form-group">
										...
										<div class="col-lg-9">
						<input type="number" required="required" name="candidatos_reclutados_<?php echo $cnt1; ?>" id="candidatos_reclutados_<?php echo $cnt1; ?>" class="form-control" placeholder="Indique el número de candidatos reclutados." value="<?php echo KT_escapeAttribute($row_rsvac_vacante['candidatos_reclutados']); ?>">
										</div>
									</div>
									<!-- /basic text input -->
                                    <?php }  ?>
						...
						<input type="number" name="sueldo_<?php echo $cnt1; ?>" id="sueldo_<?php echo $cnt1; ?>" class="form-control" placeholder="Ingresa el sueldo autorizado."  required="required" value="<?php echo KT_escapeAttribute($row_rsvac_vacante['sueldo']); ?>">
									...
									<div class="form-group">
										...
										<div class="col-lg-9">
											<select name="IDfuente_<?php echo $cnt1; ?>" id="IDfuente_<?php echo $cnt1; ?>" class="form-control" required="required">
												<option value="">Seleccione una opción</option> 
													  <?php  do { ?>
