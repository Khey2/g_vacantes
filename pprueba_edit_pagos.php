...
if($tipo_nomina == 1) {
mysql_select_db($database_vacantes, $vacantes);
$query_calendario = "SELECT * FROM pp_prueba_semanas WHERE IDtipo = $tipo_nomina AND fecha_pago > DATE_SUB(NOW(),INTERVAL 5 MONTH) LIMIT 50";  
$calendario = mysql_query($query_calendario, $vacantes) or die(mysql_error());
$row_calendario = mysql_fetch_assoc($calendario);
} else {
mysql_select_db($database_vacantes, $vacantes);
$query_calendario = "SELECT * FROM pp_prueba_semanas WHERE IDtipo = $tipo_nomina AND fecha_pago > DATE_SUB(NOW(),INTERVAL 5 MONTH) LIMIT 25";  
$calendario = mysql_query($query_calendario, $vacantes) or die(mysql_error());
$row_calendario = mysql_fetch_assoc($calendario);
}
