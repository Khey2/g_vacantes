...
<!-- Content area -->
				<div class="content">
                  ...
                  <div class="panel panel-flat">
                    ...
                    <th class="bg-warning">Pull Vacaciones</th>
                    </tr> 
                    </thead>
                    <tbody>
                    <?php 
					...
					do {
						...
						mysql_select_db($database_vacantes, $vacantes);
						$query_global_tempv = "SELECT vac_vacante.IDvacante, vac_vacante.ajuste_dias, vac_puestos.dias, vac_areas.area, vac_vacante.IDestatus,  vac_vacante.IDmotivo_v, vac_vacante.fecha_requi, vac_matriz.matriz FROM vac_vacante LEFT JOIN vac_areas ON vac_areas.IDarea = vac_vacante.IDarea LEFT JOIN vac_puestos ON vac_puestos.IDpuesto = vac_vacante.IDpuesto INNER JOIN vac_matriz ON vac_matriz.IDmatriz = vac_vacante.IDmatriz WHERE vac_vacante.IDestatus = 1 AND vac_matriz.IDmatriz = '$IDmatriz' AND vac_vacante.IDarea IN ($mis_areas) AND vac_vacante.IDmotivo_v = 5";
						$global_tempv = mysql_query($query_global_tempv, $vacantes) or die(mysql_error());
						$row_global_tempv = mysql_fetch_assoc($global_tempv);
						$totalRows_global_tempv = mysql_num_rows($global_tempv);
						...
						do { 
						 ...
						 if ($previo < 4) {$antes_tiempo = $antes_tiempo + 1;}
						 
						 else if ($previo <  $row_global['dias']) { $a_tiempo = $a_tiempo + 1; }
						 
						 else if ($previo < $row_global['dias'] + 4) {$fuera_tiempo = $fuera_tiempo + 1;} 

						 else if ($previo >= $row_global['dias'] + 4) {$muy_fuera_tiempo = $muy_fuera_tiempo + 1;  }
						 
						 if ($row_global['IDmotivo_v'] == 3) {$vacantes_pull = $vacantes_pull + 1; }

						 if ($row_global['IDmotivo_v'] == 5) {$vacantes_pullv = $vacantes_pullv + 1; }
						 
						 $activas = $antes_tiempo +  $a_tiempo +  $fuera_tiempo + $muy_fuera_tiempo - $vacantes_pull;
						...
						$Xvacantes_pullv = $Xvacantes_pullv + $vacantes_pullv;

					
					?>
                    <tr>
                    <td><form method="POST" action="vista_activas.php"><input type="hidden" name="la_matriz[]" id="la_matriz[]" value="<?php echo $IDmatriz;?>" />
                    ...
                    <td align="center"><?php echo $activas; ?></td>
                    ...
                    <td align="center"><?php  echo $totalRows_global_tempv; ?></td>
                    </tr>
                    <?php } while($row_lmatriz = mysql_fetch_array($lmatriz)); ?>
                    <tr>
                    <td class="success"><strong>TOTAL</strong></td>
                    <td align="center" class="success"><strong><?php echo $Xactivas; ?></strong></td>
                    ...
                    <td align="center" class="success"><strong><?php echo $Xvacantes_pull; ?></strong></td>
                    <td align="center" class="success"><strong><?php echo $Xvacantes_pullv; ?></strong></td>
                    </tr>
                    </tbody>
                   </table> 
