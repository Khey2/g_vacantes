...
if (isset($_SESSION['la_region']) AND $_SESSION['la_region'] == 1) {$e1 = ' AND prod_captura.IDmatriz IN (6, 8, 9, 11, 12, 14, 21, 23, 24)'; }
else if (isset($_SESSION['la_region']) AND $_SESSION['la_region'] == 2) {$e1 = ' AND prod_captura.IDmatriz IN (1, 3, 4, 10, 15, 19, 20, 25, 30)'; }
else if (isset($_SESSION['la_region']) AND $_SESSION['la_region'] == 3) {$e1 = ' AND prod_captura.IDmatriz IN (2, 13, 16, 17, 18, 22, 26, 28, 29)'; }
else {$e1 = '';}
...
mysql_select_db($database_vacantes, $vacantes);
$query_reporte = "SELECT prod_captura.IDempleado, prod_captura.emp_paterno, prod_captura.emp_materno, prod_captura.emp_nombre, prod_captura.denominacion, prod_captura.sueldo_total, prod_captura.sueldo_total, prod_captura.garantizado, prod_captura.adicional, prod_captura.adicional, prod_captura.reci, prod_captura.carg, prod_captura.dist, prod_captura.esti, prod_captura.pago, prod_captura.pago_total, prod_captura.capturador, prod_captura.validador, prod_captura.autorizador,  prod_captura.lun, prod_captura.mar, prod_captura.mie, prod_captura.jue, prod_captura.vie, prod_captura.sab, prod_captura.dom, vac_matriz.matriz, prod_captura.IDpuesto, prod_captura.autorizador, prod_captura.bono_asistencia, prod_captura.adicional3, prod_captura.horas_extra, prod_captura.horas_extra_monto,	prod_semana_cierre.captura, prod_semana_cierre.valida, 	prod_semana_cierre.autoriza FROM prod_captura LEFT JOIN vac_matriz ON vac_matriz.IDmatriz = prod_captura.IDmatriz LEFT JOIN prod_semana_cierre ON prod_captura.semana = prod_semana_cierre.semana AND
prod_captura.anio = prod_semana_cierre.anio AND prod_captura.IDmatriz = prod_semana_cierre.IDmatriz WHERE prod_captura.anio = '$anio' ".$a1.$b1.$c1.$d1.$e1." AND prod_semana_cierre.captura = 1
AND prod_semana_cierre.valida = 1 AND prod_semana_cierre.autoriza = 1 ORDER BY vac_matriz.IDmatriz ASC";
...
$objPHPExcel = $objReader->load("PRD/cedulaT.xlsx");
  ...
  if ($row_reporte['horas_extra'] == '') { $horas_extra = 0;} else  { $horas_extra = $row_reporte['horas_extra'];}
	...
	if (in_array($el_puesto, $AuxiliaresAlmacen))
         { $pago_total_f = $row_reporte[8] + $row_reporte[15] + $row_reporte[29] + $row_reporte[30] + $horas_extra_monto; }
    else { $pago_total_f = $row_reporte[9] + $row_reporte[15] + $row_reporte[29] + $horas_extra_monto;}
  ...
  if($row_reporte[10] > 0) { $rec1 = $row_reporte[10]; } else { $rec1 = 0; }  
	...
	if($row_reporte[30] > 0) { $rec5 = $row_reporte[30]; } else { $rec5 = 0; }  

	//$Pre_final = $adicional + $row_reporte[15] + $row_reporte[30];

	$Pre_final = $adicional + $row_reporte['pago_total'] + $row_reporte['adicional3'];

        $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $rec5); 
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $bono_asistencia); 
        $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $horas_extra); 
        $objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $horas_extra_monto); 
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('S'.$rowCount, $row_reporte[15]); 
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('Z'.$rowCount, $row_reporte[26]); 
