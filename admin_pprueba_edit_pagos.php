...
$fecha_fin = $row_empleado['fecha_fin'];
$fecha_inicio = $row_empleado['fecha_inicio'];
$IDpuesto_destino = $row_empleado['IDpuesto_destino'];

mysql_select_db($database_vacantes, $vacantes);
$query_pprueba = "SELECT * FROM pp_prueba_pagos WHERE IDpprueba = $IDpprueba ORDER BY semana DESC";  
...
mysql_select_db($database_vacantes, $vacantes);
$query_calendario = "SELECT * FROM pp_prueba_semanas WHERE IDtipo = $tipo_nomina AND fecha_pago > CURDATE() LIMIT 100";  
...
$semana_pago = $_POST['fecha_pago']; 
$query_sem_inicio = "SELECT * FROM pp_prueba_semanas WHERE semana = '$semana_pago' AND IDtipo = $tipo_nomina";  
$sem_inicio = mysql_query($query_sem_inicio, $vacantes) or die(mysql_error());
$row_sem_inicio = mysql_fetch_assoc($sem_inicio);
$fecha_paggo = $row_sem_inicio['fecha_pago'];

$insertSQL = sprintf("INSERT INTO pp_prueba_pagos (IDempleado, IDpprueba, fecha_pago, monto_pago, semana, IDestatus) VALUES (%s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($IDempleado, "int"),
                       GetSQLValueString($IDpprueba, "int"),
                       GetSQLValueString($fecha_paggo, "text"),
 ...
 header("Location: admin_pprueba_edit_pagos.php?IDpprueba=$IDpprueba&info=1");
}
...
$updateSQL = sprintf("UPDATE pp_prueba_pagos SET fecha_pago=%s, monto_pago=%s, semana=%s, IDestatus=%s WHERE IDpprueba_pagos=%s",
                       GetSQLValueString($fecha_paggo, "text"),
                       GetSQLValueString($_POST['monto_pago'], "text"),
                       GetSQLValueString($_POST['fecha_pago'], "text"),
                       GetSQLValueString($_POST['IDestatus'], "int"),
...
$dif_sueldo_tope = $sueldo_actual * 0.35;

// si es supervisor
$search_array = array(17, 56, 58, 193, 198, 235, 270, 359, 373);
if (in_array($IDpuesto_destino, $search_array)) {$dif_sueldo = $dif_sueldo; $noes = 1;}
if(($dif_sueldo > $dif_sueldo_tope) AND $noes == 0) {$dif_sueldo = $dif_sueldo_tope;}
...
$fecha_pago = date("Y-m-d", strtotime($row_empleado['fecha_inicio']."+ 1 month")); 

$bulce = 0;
while ($bulce < $meses){
//encontrar fecha de pago
$query_sem_inicio = "SELECT * FROM pp_prueba_semanas WHERE IDtipo = '$tipo_nomina' AND fecha_pago > '".$fecha_pago."' ORDER BY ABS(DATEDIFF(fecha_pago, '".$fecha_pago."')) ASC LIMIT 1";  
$sem_inicio = mysql_query($query_sem_inicio, $vacantes) or die(mysql_error());
$row_sem_inicio = mysql_fetch_assoc($sem_inicio);
$fecha_paggo = $row_sem_inicio['fecha_pago'];
$semana_paggo = $row_sem_inicio['semana'];

$insertSQL = "INSERT INTO pp_prueba_pagos (IDempleado, IDpprueba, fecha_pago, monto_pago, semana, IDestatus) VALUES ($IDempleado, $IDpprueba, '$fecha_paggo', '$dif_sueldo', '$semana_paggo', '0')";
mysql_select_db($database_vacantes, $vacantes);
$Result1 = mysql_query($insertSQL, $vacantes) or die(mysql_error());
...
header("Location: admin_pprueba_edit_pagos.php?IDpprueba=$IDpprueba&info=9");
}
...
if ((isset($_GET['IDpprueba_borrar'])) && ($_GET['IDpprueba_borrar'] != "")) {
 ...
 header("Location: admin_pprueba_edit_pagos.php?IDpprueba=$IDpprueba&info=3");
}
			...
			<?php require_once('assets/pheader.php'); ?>
				...
				<div class="content">
					...
					<div class="panel-body text-center">
                     ...
                     <a class="btn btn-info" href="admin_pprueba_edit_pagos.php?IDpprueba=<?php echo $IDpprueba; ?>&pregarca=1">Precargar Pagos<i class="icon-arrow-right14 position-right"></i></a>
                     <?php } ?>
                                ...
                                <tbody>
								  ...
								  <?php do { ?>
                                    <tr>
                                      ...
                                      <td><?php echo $row_pprueba['semana'];?></td>
                                      <td><?php echo round($row_pprueba['monto_pago'],2); ?></td>
										...
										<div class="col-lg-9">
											<select name="fecha_pago" id="fecha_pago" class="form-control"  required="required">
												<option value="">Seleccione una opción</option> 
												  <?php  do { ?>
												  <option value="<?php echo $row_calendario['semana']?>"<?php if (!(strcmp($row_calendario['semana'], $row_pprueba['semana']))) 
									...
									<div id="modal_theme_danger<?php echo $row_pprueba['IDpprueba_pagos']; ?>" class="modal fade" tabindex="-1">
										<div class="modal-dialog">
											<div class="modal-content">
												...
												<div class="modal-footer">
													<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
													<a class="btn btn-danger" href="admin_pprueba_edit_pagos.php?IDpprueba=<?php echo $IDpprueba; ?>&IDpprueba_borrar=<?php echo $row_pprueba['IDpprueba_pagos']; ?>">Si borrar</a>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-lg-9">
											<select name="fecha_pago" id="fecha_pago" class="form-control"  required="required">
												<option value="">Seleccione una opción</option> 
												  <?php  do { ?>
												  <option value="<?php echo $row_calendario['semana']?>"><?php echo $row_calendario['semana']?></option>
												  <?php
												 } while ($row_calendario = mysql_fetch_assoc($calendario));
												   $rows = mysql_num_rows($calendario);
												   if($rows > 0) {
