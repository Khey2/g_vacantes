...
mysql_select_db($database_vacantes, $vacantes);
$query_reporte = "SELECT prod_activos.IDempleado, prod_activos.emp_paterno, prod_activos.emp_materno, prod_activos.emp_nombre, vac_puestos.denominacion, prod_activos.sueldo_diario, inc_motivos.motivo, inc_captura.inc3, inc_captura.inc6, inc_captura.validador, inc_captura.obs3,  prod_activos.descripcion_nomina, inc_captura.transporte_monto FROM prod_activos LEFT JOIN vac_puestos ON prod_activos.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_matriz ON prod_activos.IDmatriz = vac_matriz.IDmatriz INNER JOIN inc_captura ON inc_captura.IDempleado = prod_activos.IDempleado AND inc_captura.semana = '$semana'  AND inc_captura.anio = '$anio' LEFT JOIN inc_motivos ON inc_captura.IDmotivo3 = inc_motivos.IDmotivo WHERE prod_activos.IDmatriz = '$IDmatriz' AND (inc_captura.inc3 > 0 OR inc_captura.inc6 > 0 or inc_captura.transporte > 0)";
...
// Create new PHPExcel object
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Festivos');
    $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Monto Fest.');
    $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Monto Transporte');
    $objPHPExcel->getActiveSheet()->SetCellValue('L1', 'Validado');
    $objPHPExcel->getActiveSheet()->SetCellValue('M1', 'Observaciones');
    $objPHPExcel->getActiveSheet()->SetCellValue('N1', 'Empresa');
...
$rowCount = 2; //new
	...
	if($row_reporte[9] == "") { $validado = "NO"; } else { $validado = "SI"; }
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $row_reporte[12]); 
        $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $validado); 
        $objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $row_reporte[10]); 
        $objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, $row_reporte[11]); 
		
		$objPHPExcel->getActiveSheet()->getStyle('A1:N1')->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle('H2:H500')->getNumberFormat()->setFormatCode('"$"#,##0.00_-');
		$objPHPExcel->getActiveSheet()->getStyle('K2:K500')->getNumberFormat()->setFormatCode('"$"#,##0.00_-');
		$objPHPExcel->getActiveSheet()->getStyle('H2:H500')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
// Increment the Excel row counter
    ...
    header('Content-Disposition: attachment;filename="Incentivos '.date('dmY') . " " . $matriz . '.xls"');
