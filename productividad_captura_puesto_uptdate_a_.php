...
// dias que cubre
if ($_res1  + $_res2  + $_res4  < $minimos) { $cuota_lun = 0;} else { $cuota_lun = 1;}
if ($_res5  + $_res6  + $_res8  < $minimos) { $cuota_mar = 0;} else { $cuota_mar = 1;}
if ($_res9  + $_res10 + $_res12 < $minimos) { $cuota_mie = 0;} else { $cuota_mie = 1;}
if ($_res13 + $_res14 + $_res16 < $minimos) { $cuota_jue = 0;} else { $cuota_jue = 1;}
if ($_res17 + $_res18 + $_res20 < $minimos) { $cuota_vie = 0;} else { $cuota_vie = 1;}
if ($_res21 + $_res22 + $_res24 < $minimos) { $cuota_sab = 0;} else { $cuota_sab = 1;}
if ($_res25 + $_res26 + $_res28 < $minimos) { $cuota_dom = 0;} else { $cuota_dom = 1;}
...
$essab =  $_res23 * $estib;
$esdom =  $_res27 * $estib;
$total_estiba = $eslun + $esmar + $esmie + $esjue + $esvie + $essab + $esdom;

echo "estiba lun: " .$eslun . "</br>";	
echo "estiba mar: " .$esmar . "</br>";	
echo "estiba mie: " .$esmie . "</br>";	
echo "estiba jue: " .$esjue . "</br>";	
echo "estiba vie: " .$esvie . "</br>";	
echo "estiba sab: " .$essab . "</br>";	
echo "estiba dom: " .$esdom . "</br></br>";	
echo "estiba total: " .$total_estiba . "</br></br>";	

// montos
if ($cuota_lun == 1) {$montolun = ($ellun * $otros) + $eslun;} else { $montolun = 0;}
if ($cuota_mar == 1) {$montomar = ($elmar * $otros) + $esmar;} else { $montomar = 0;}
if ($cuota_mie == 1) {$montomie = ($elmie * $otros) + $esmie;} else { $montomie = 0;}
if ($cuota_jue == 1) {$montojue = ($eljue * $otros) + $esjue;} else { $montojue = 0;}
if ($cuota_vie == 1) {$montovie = ($elvie * $otros) + $esvie;} else { $montovie = 0;}
if ($cuota_sab == 1) {$montosab = ($elsab * $otros) + $essab;} else { $montosab = 0;}
if ($cuota_dom == 1) {$montodom = ($eldom * $otros) + $esdom;} else { $montodom = 0;}
...
$productividad_diaria = $sueldo_diario * 0.3;
...
$dias_laborados = $lun_a + $mar_a + $mie_a + $jue_a + $vie_a + $sab_a + $dom_a;
	if ($dias_laborados == 7) {$asistencia = 1; } 
elseif ($dias_laborados == 6) {$asistencia = 1; } 
else 						  {$asistencia = 0; } 

// si no cubre con asistencia de 6 dias
if ($asistencia == 0 ) { $suma = 0;}
echo "Pago Luego de asistencia: " . $suma . "</br>";	
...
$monto_T = $monto + $monto_adicional + $garantia_monto + $total_estiba;
...
echo "Distrib: " . $dist . "</br>";
    ...
    $query1 = "UPDATE prod_captura SET pago = '$porc_pago', pago_total = '$monto_T', reci = '$reci', carg = '$carg', esti = '$esti', dist = '$dist' WHERE IDcaptura = '$IDcaptura'"; 
    $result1 = mysql_query($query1) or die(mysql_error());  
	
	//redirecto
	header("Location: productividad_captura_puesto_a_.php?IDpuesto=$el_puesto"); 	
				
	//cubre con asistencia y puntualidad
	}  else if ($garantizado == 0 && $asistencia == 1) {
    ...
    $query2 = "UPDATE prod_captura SET pago_total = '0', reci = '$reci', carg = '$carg', esti = '$esti', dist = '$dist'  WHERE IDcaptura = '$IDcaptura'"; 
