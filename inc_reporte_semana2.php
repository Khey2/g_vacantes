...
mysql_select_db($database_vacantes, $vacantes);
$query_reporte = "SELECT prod_activos.IDempleado, prod_activos.emp_paterno,  prod_activos.emp_materno, prod_activos.emp_nombre, vac_puestos.denominacion, prod_activos.sueldo_diario, inc_captura_2020.dias2, inc_captura_2020.horas2, inc_motivos.motivo, inc_captura_2020.inc2, inc_captura_2020.validador, inc_captura_2020.obs2, prod_activos.descripcion_nomina FROM prod_activos LEFT JOIN vac_puestos ON prod_activos.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_matriz ON prod_activos.IDmatriz = vac_matriz.IDmatriz INNER JOIN inc_captura_2020 ON inc_captura_2020.IDempleado = prod_activos.IDempleado AND inc_captura_2020.semana = '$semana' LEFT JOIN inc_motivos ON inc_captura_2020.IDmotivo2 = inc_motivos.IDmotivo WHERE prod_activos.IDmatriz = '$IDmatriz' AND inc_captura_2020.inc2 > 0";
...
mysql_select_db($database_vacantes, $vacantes);
$query_reporte = "SELECT prod_activos.IDempleado, prod_activos.emp_paterno,  prod_activos.emp_materno, prod_activos.emp_nombre, vac_puestos.denominacion, prod_activos.sueldo_diario, inc_captura.dias2, inc_captura.horas2, inc_motivos.motivo, inc_captura.inc2, inc_captura.validador, inc_captura.obs2, prod_activos.descripcion_nomina FROM prod_activos LEFT JOIN vac_puestos ON prod_activos.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_matriz ON prod_activos.IDmatriz = vac_matriz.IDmatriz INNER JOIN inc_captura ON inc_captura.IDempleado = prod_activos.IDempleado AND inc_captura.semana = '$semana'  AND inc_captura.anio = '$anio' LEFT JOIN inc_motivos ON inc_captura.IDmotivo2 = inc_motivos.IDmotivo WHERE prod_activos.IDmatriz = '$IDmatriz' AND inc_captura.inc2 > 0";
mysql_query("SET NAMES 'utf8'"); 
$reporte = mysql_query($query_reporte, $vacantes) or die(mysql_error());
...
// Create new PHPExcel object
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Dias');
    $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Horas');
    $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Motivo');
    $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Monto');
    $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Validado');
	...
	if($row_reporte[10] == "") { $validado = ""; } else { $validado = "SI"; }
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $validado); 
		...
		$objPHPExcel->getActiveSheet()->getStyle('J2:J500')->getNumberFormat()->setFormatCode('"$"#,##0.00_-');
		$objPHPExcel->getActiveSheet()->getStyle('J2:J500')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
// Increment the Excel row counter
    ...
    header('Content-Disposition: attachment;filename="Compensacion por Suplencia '.date('dmY') . " " . $matriz . '.xls"');
