...
if (isset($_POST['el_grupo'])) {$_SESSION['el_grupo'] = $_POST['el_grupo'];} else {$_SESSION['el_grupo'] = 2;}
...
$query_resultados = "SELECT
	...
	sed_competencias_resultados.IDgrupo,
	prod_activos_evaluado.IDempleado AS IDempleadoO,
	prod_activos_evaluado.emp_paterno AS emp_paternoO,
	prod_activos_evaluado.emp_materno AS emp_maternoO,
	prod_activos_evaluado.emp_nombre AS emp_nombreO,
	prod_activos_evaluado.denominacion AS denominacionO,
	prod_activos_evaluador.IDempleado AS IDempleadoOr,
	prod_activos_evaluador.emp_paterno AS emp_paternoOr,
	prod_activos_evaluador.emp_materno AS emp_maternoOr,
	prod_activos_evaluador.emp_nombre AS emp_nombreOr,
	prod_activos_evaluador.denominacion AS denominacionOr 
FROM
	sed_competencias_resultados
	LEFT JOIN prod_activos AS prod_activos_evaluado ON sed_competencias_resultados.IDempleado = prod_activos_evaluado.IDempleado
	LEFT JOIN prod_activos AS prod_activos_evaluador ON prod_activos_evaluador.IDempleado = sed_competencias_resultados.IDempleado_evaluador 
WHERE
	prod_activos_evaluado.IDempleado IS NULL OR	prod_activos_evaluador.IDempleado IS NULL  ";
...
<div class="content">
                  ...
                  <div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">360° Rerpote de Avance</h5>
						</div>

					<div class="panel-body">
							<p>Filtra por Grupo de evaluación y estatus.</p>
					...
					<table class="table">
						<tbody>							  
                         ...
                         <select name="el_estatus" class="form-control">
                           <option value="1"<?php if (!(strcmp($el_estatus, 1))) {echo "selected=\"selected\"";} ?>>Evaluado</option>
                           <option value="0"<?php if (!(strcmp($el_estatus, 0))) {echo "selected=\"selected\"";} ?>>Sin Evaluación</option>
                           <option value="2"<?php if (!(strcmp($el_estatus, 2))) {echo "selected=\"selected\"";} ?>>Todos</option>
						</select>
                            </td>
							<td>
					  ...
					  ?>
                        <tr>
                          ...
                          <td><?php  if ($row_resultados['IDestatus'] == 1) { echo "<span class='label label-success'>Evaluado</span>";} 
							 ...
							 ?></td>
                          <td><?php  echo $totalRows_evaluaciones_ok." de ".$totalRows_evaluaciones; ?>
						  
					 <button type="button" data-target="#modal_theme_danger<?php echo $row_resultados['IDempleado']; ?>2"  data-toggle="modal" class="btn btn-xs btn-danger icon-trash"></button>                          
