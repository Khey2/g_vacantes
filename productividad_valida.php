...
//total gastado
mysql_select_db($database_vacantes, $vacantes);
$query_montoX1 = "SELECT Sum(prod_captura.pago_total) As Monto, prod_captura.IDmatriz, Sum(prod_captura.adicional2) AS Monto2, Sum(prod_captura.bono_asistencia) AS Monto5, Sum(prod_captura.adicional3) AS Monto6, Sum(prod_captura.adicional) AS Monto4, Sum(prod_captura.horas_extra_monto) AS MontoHE, Sum(prod_captura.sueldo_total) AS Monto3, prod_captura.semana FROM prod_captura WHERE  prod_captura.anio = '$anio_previo' AND prod_captura.semana = '$semana_previa' AND prod_captura.IDmatriz = '$IDmatriz' GROUP BY prod_captura.IDmatriz, prod_captura.semana ";
...
//repetidos
mysql_select_db($database_vacantes, $vacantes);
$query_repetidos = "SELECT Count(prod_captura.IDempleado) AS Repetidos FROM prod_captura WHERE prod_captura.IDmatriz = '$la_matriz' AND prod_captura.semana = '$semana' AND prod_captura.anio = '$anio' GROUP BY prod_captura.IDempleado HAVING Repetidos > 1";
...
<div class="content">
					    ...
					    <div class="alert bg-danger-300 alert-styled-left">
							<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Cerrar</span></button>
							Estimado Usuario, ya puedes proceder con la Validación. 
					    </div>
                        <?php } ?>
					    <!-- /basic alert -->
					    ...
					    <div class="alert bg-success-300 alert-styled-left">
							<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Cerrar</span></button>
							Se ha <span class="text-semibold">Validado</span> la semana de forma correcta. 
					    </div>
						
                        <?php } else if($row_cierre['estatus'] != 1) { ?>
					    <div class="alert bg-success-300 alert-styled-left">
							<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Cerrar</span></button>
							Debes cerrar la <span class="text-semibold">Validación </span> de la semana, dando clic en "Solicitar Autorización". 
					    </div>
                        <?php } ?>
					    ...
					    <div class="alert bg-success-300 alert-styled-left">
							<button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Cerrar</span></button>
							Se han <span class="text-semibold">Enviado</span> la solicitud de Autorizacion. 
					    </div>
                        <?php } ?>
					    <!-- /basic alert -->
					...
					<div class="row">
							...
							<div class="panel panel-body">
								<div class="media no-margin-top content-group">
									...
									<div class="media-right media-middle">
										<i class="icon-cash3 icon-2x text-info-400 opacity-75"></i>
									</div>
								</div>

								<div class="progress progress-micro mb-10">
									<div class="progress-bar bg-info-400" style="width: 100%">
									</div>
								</div>
						...
						<div class="col-sm-6 col-md-3">
							<div class="panel panel-body">
								<div class="media no-margin-top content-group">
									...
									<div class="media-right media-middle">
										<i class="icon-cash2 icon-2x text-info-400 opacity-75"></i>
									</div>
								</div>
							...
							<div class="panel panel-body">
								<div class="media no-margin-top content-group">
									<div class="media-left media-middle">
										<i class="icon-calendar2 icon-2x text-info-400 opacity-75"></i>
									</div>
								...
								<div class="progress progress-micro mb-10">
									<div class="progress-bar bg-info" style="width: 100%">
										...
										<span class="text-muted"><strong>Mes:</strong> <?php echo $elmes ."<br/>"; ?></span>
						...
						<div class="col-sm-6 col-md-3">
							<div class="panel panel-body">
								<div class="media no-margin-top content-group">
									<div class="media-left media-middle">
										<i class="icon-users2  icon-2x text-info-400 opacity-75"></i>
									</div>
					...
					<!-- /statistics with progress bar -->
		                       ...
		                       <form method="POST" action="productividad_valida.php">
					...
					<?php if ($row_cierre['captura'] == 1) { ?>
									<a href="prod_valida_sucursal.php" class="btn bg-success-400"><i class="icon-checkmark-circle2 position-left"></i>Validar Sucursal</a>

    				<?php if ($rebase == 0) { ?> 
								<button type="button" data-target="#modal_theme_danger"  data-toggle="modal" class="btn bg-primary-400"><i class="icon-notification2 position-left"></i> Solicitar Autorización</button></a>
    				<?php } else { ?> 
								<button type="button" class="btn bg-primary-400" id="sweet_error"><i class="icon-notification2 position-left"></i> Solicitar Autorización</button></td>
    				<?php }  ?> 
                                    <?php } ?>
                                 
					<?php if ( $IDmatriz  !=  25 ) { ?>							
                            <a href="productividad_reportes_new_valida.php?IDmatriz=<?php echo $la_matriz; ?>&anio=<?php echo $anio; ?>&semana=<?php echo $semana; ?>&areas=0&tipo=0" class="btn bg-info-400"><i class="icon-file-excel position-left"></i>Descargar Reporte</a>
					<?php } else { ?> 
                    ...
                    <tbody>
										...
										?>
									      <tr>
                                            ...
                                            onClick="window.location.href='productividad_valida_puesto_t.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>'"><i class="icon-arrow-right6"></i> Ver Empleados</button>
                                            <?php } else { ?>
                                            <button type="button" class="btn btn-info btn-xs" 
                                            onClick="window.location.href='productividad_valida_puesto_a_t.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>'"><i class="icon-arrow-right6"></i> Ver Empleados</button>
                                            <?php } ?>
                                            <button type="button" class="btn btn-success btn-xs" 
                                            onClick="window.location.href='prod_valida_puesto.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>&IDarea=<?php echo $row_detalle['IDarea']; ?>&IDsucursal=<?php echo $row_detalle['IDsucursal']; ?>'"><i class="icon-checkmark-circle2"></i> Validar Puesto</button>
											...
											<?php if ( $row_detalle['modal'] == 100 ) { ?>
                                            <button type="button" class="btn btn-info btn-xs" 
                                            onClick="window.location.href='productividad_valida_puesto.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>'"><i class="icon-arrow-right6"></i> Ver Empleados</button>
                                            <?php } else { ?>
                                            <button type="button" class="btn btn-info btn-xs" 
                                            onClick="window.location.href='productividad_valida_puesto_a.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>'"><i class="icon-arrow-right6"></i> Ver Empleados</button>
                                            <?php } ?>
                                            <button type="button" class="btn btn-success btn-xs" 
                                            onClick="window.location.href='prod_valida_puesto.php?IDpuesto=<?php echo $row_detalle['IDpuesto']; ?>&IDarea=<?php echo $row_detalle['IDarea']; ?>&IDsucursal=<?php echo $row_detalle['IDsucursal']; ?>'"><i class="icon-checkmark-circle2"></i> Validar Puesto</button>
									        ...
									        <td><?php echo $row_empleados_c['TCapturados']; ?></td>
									        <td><?php echo $row_empleados_c['TValidados']; ?><?php if(($row_empleados_c['TCapturados'] == $row_empleados_c['TValidados']) AND $row_empleados_c['TValidados'] > 0){ ?>&nbsp;<i class='icon-checkmark5 icon-xs text-success'></i> <?php } ?></td>
      						              </tr>
									      <?php 
										  $Total_a = $Total_a + $row_detalle['Ocupados'];
										  $Total_b = $Total_b + $row_empleados_c['TCapturados'];
										  $Total_c = $Total_c + $row_empleados_c['TValidados'];
										  } while ($row_detalle = mysql_fetch_assoc($detalle)); ?>
									<tfoot> 
									<tr> 
									  <th colspan="4">Total</th>
									  <th class="text text-center"><?php echo $Total_a; ?></th>
									  <th class="text text-center"><?php echo $Total_b; ?></th>
									  <th class="text text-center"><?php echo $Total_c; ?></th>
									 </tr>
									</tfoot>
                                        </tbody>
                                       </table> 
