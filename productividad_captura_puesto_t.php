...
if ($_POST['horas_extra'] == ''){$horas_extra = 0;} else {$horas_extra = $_POST['horas_extra'];}
  ...
  $updateGoTo = "productividad_captura_puesto_update_t.php?IDcaptura=$captura";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (!isset($_POST['a3'])) {$_a3 = 0;} else {$_a3 = $_POST['a3'];}
if (!isset($_POST['a4'])) {$_a4 = 0;} else {$_a4 = $_POST['a4'];}
if (!isset($_POST['a5'])) {$_a5 = 0;} else {$_a5 = $_POST['a5'];}
if (!isset($_POST['a6'])) {$_a6 = 0;} else {$_a6 = $_POST['a6'];}
if (!isset($_POST['a7'])) {$_a7 = 0;} else {$_a7 = $_POST['a7'];}
if (!isset($_POST['a25'])) {$_a25 = 0;} else {$_a25 = $_POST['a25'];}
if (!isset($_POST['a26'])) {$_a26 = 0;} else {$_a26 = $_POST['a26'];}
if (!isset($_POST['a27'])) {$_a27 = 0;} else {$_a27 = $_POST['a27'];}
...
if ($_POST['horas_extra'] == ''){$horas_extra = 0;} else {$horas_extra = $_POST['horas_extra'];}
  ...
  $insertSQL = sprintf("INSERT INTO prod_captura (IDempleado, emp_paterno, emp_materno, emp_nombre, denominacion, sueldo_total, IDpuesto, fecha_captura, semana, anio,  IDmatriz, IDsucursal, IDarea, IDturno, a1, a2, a3, a4, a5, a6, a7, a25, a26, a27, a28, horas_extra, capturador, garantizado, adicional, observaciones, lun, mar, mie, jue, vie, sab, dom) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($_POST['garantizado'], "int"),
                       GetSQLValueString($_adicional, "int"),
                       GetSQLValueString($_observaciones, "text"),
                       GetSQLValueString(isset($_POST['lun']) ? "true" : "", "defined","1","0"),
                       GetSQLValueString(isset($_POST['mar']) ? "true" : "", "defined","1","0"),
                       GetSQLValueString(isset($_POST['mie']) ? "true" : "", "defined","1","0"),
                       GetSQLValueString(isset($_POST['jue']) ? "true" : "", "defined","1","0"),
  ...
  $insertGoTo = "productividad_captura_puesto_update_t.php?IDcaptura=$captura";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_vacantes, $vacantes);
$query_turnol = "SELECT * FROM vac_turnos_t WHERE IDturno = $el_turno";
$turnol = mysql_query($query_turnol, $vacantes) or die(mysql_error());
$row_turnol = mysql_fetch_assoc($turnol);
$totalRows_turnol = mysql_num_rows($turnol);
...
$updateGoTo = "productividad_captura_puesto_t.php?IDpuesto=".$el_puesto."&info=1";
if (isset($_SERVER['QUERY_STRING'])) {
  $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
  $updateGoTo .= $_SERVER['QUERY_STRING'];
}
...
<!-- Page container -->
	<div class="page-container">
		...
		<?php require_once('assets/menu.php'); ?>
            ...
            <?php if((isset($_GET['info']) && ($_GET['info'] == 1))) { ?>
              ...
              <!-- Basic alert -->
                    ...
                    <label class="control-label no-margin text-semibold">Sucursal:</label>
											...
											<?php echo $row_turnol['turno']; ?>

						</div>

					<div class="panel-body"> 
                    <p>Selecciona el nombre del empleado para ver su histórico de pago de productividad. Da clic en el botón para capturar su productividad.</p>
          ...
          <p>Da clic en el incono de <i class=" icon-sort-time-asc text-info"></i> para cambiar el turno del colaborador.</p>

      <form method="POST" name="form3" action="productividad_captura_puesto_t.php?IDpuesto=<?php echo $el_puesto; ?>">
      <input type="hidden" name="MM_update" value="form3">                                   
      <div class="panel-body text-center alpha-grey">								
          <div class="form-group col-md-2">
            <select name="el_turno" class="form-control ">
											<?php do { ?>
                    <option value="<?php echo $row_turno['IDturno']?>"<?php if (!(strcmp($row_turno['IDturno'], $el_turno))) 
      ...
      <div class="table-responsive">
					<table class="table table-condensed datatable-button-html5-columns">
                        ...
                        <?php do { ?>
                         ...
                         <div onClick="loadDynamicContentModal('<?php echo $row_puestos['IDempleado']; ?>')" class="btn btn-success btn-icon"><i class="icon-arrow-right6"></i> Capturar</div>                 
                        <?php } else { ?>
                         <div onClick="loadDynamicContentModal('<?php echo $row_puestos['IDempleado']; ?>')" class="btn btn-primary btn-icon"><i class="icon-arrow-right6"></i> Actualizar</div>
                        <?php } ?>
                          </td>  
                            <td><?php echo $row_puestos['IDempleado']; ?>
                            <div onClick="loadDynamicContentModal_t('<?php echo $row_puestos['IDempleado']; ?>')" class="btn btn-icon"><i class="icon-sort-time-asc text-info"></i></div>
                            </td>
                            <td><?php echo $row_puestos['IDnivel_antiguedad']; ?>
                           <td><a href="prod_empleado_detalle.php?IDempleado=<?php echo $row_puestos['IDempleado']; ?>"><?php echo $row_puestos['emp_paterno']; ?> <?php echo $row_puestos['emp_materno']; ?> <?php echo $row_puestos['emp_nombre']; ?></a></td>
                          ...
                          <?php } ?> 
                            ...
                            <td><?php $total = $row_puestos['pago_total'] + $row_puestos['adicional']  + $row_puestos['bono_asistencia'] + $row_puestos['horas_extra_monto']; echo "$". number_format($total); ?></td>
                           </tr>                         
...
function loadDynamicContentModal(modal){
	...
	$('#conte-modal').load('productividad_captura_puesto_mdl_t.php?IDpuesto=' + IDpuesto + '&semana=' + semana  + '&IDturno=' + IDturno + '&IDmatriz=' + IDmatriz + '&IDempleado='+ modal, function() {
...
function loadDynamicContentModal_t(modal){
	...
	$('#conte-modal').load('productividad_turno_captura.php?IDempleado='+ modal, function() {
		$('#bootstrap-modal').modal({show:true});
    });    
}
</script> 
