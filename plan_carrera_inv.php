...
if (isset($_POST['el_puesto'])) {	foreach ($_POST['el_puesto'] as $pueste)
	{	 $_SESSION['el_puesto'] = implode(", ", $_POST['el_puesto']);}	} 
else if(!isset($_SESSION['el_puesto'])) {$_SESSION['el_puesto'] = '2,18,41,42,43,281,282,313'; } 
$el_puesto = $_SESSION['el_puesto'];


$c1 = "";
if($el_puesto > 0) {
$c1 = " AND prod_activos.IDpuesto IN ($el_puesto)"; }
mysql_select_db($database_vacantes, $vacantes);
$query_detalle = "SELECT prod_activos.IDempleado, prod_activos.emp_paterno, prod_activos.emp_materno, prod_activos.emp_nombre, prod_activos.fecha_alta, prod_activos.denominacion, prod_activos.IDmatriz, prod_activos.IDarea, prod_activos.IDaplica_SED, pc_semaforo.IDplan, pc_semaforo.reqa, pc_semaforo.reqb, pc_semaforo.reqc, pc_semaforo.reqd, pc_semaforo.reqe, pc_semaforo.reqf, pc_semaforo.estatus, vac_puestos.plan_carrera FROM prod_activos LEFT JOIN pc_semaforo ON pc_semaforo.IDempleado = prod_activos.IDempleado LEFT JOIN vac_puestos ON vac_puestos.IDpuesto = prod_activos.IDpuesto WHERE prod_activos.IDmatriz = '$la_matriz' AND vac_puestos.plan_carrera = 1 ".$c1;   
mysql_query("SET NAMES 'utf8'");
...
$query_puestos = "SELECT DISTINCT vac_puestos.IDpuesto, vac_puestos.denominacion, vac_puestos.IDarea, vac_puestos.tipo, vac_puestos.IDaplica_PROD, vac_puestos.modal, vac_puestos.dias, vac_puestos.descrito, vac_puestos.IDdp_tipo, vac_puestos.tab, vac_puestos.prod_captura_tipo, vac_puestos.plan_carrera, prod_activos.IDmatriz FROM vac_puestos INNER JOIN prod_activos ON prod_activos.IDpuesto = vac_puestos.IDpuesto WHERE vac_puestos.plan_carrera = 1 AND prod_activos.IDmatriz = '$la_matriz' ORDER BY vac_puestos.denominacion ASC";
...
$IDpuestoPC = $_POST['IDpuestoPC'];
$fecha_licencia = $_POST['fecha_licencia'];

$updateSQL = "UPDATE pc_semaforo SET reqa = '$reqa', reqb = '$reqb', reqc = '$reqc', reqd = '$reqd', reqe = '$reqe', reqf = '$reqf', IDpuestoPC = '$IDpuestoPC', fecha_licencia = '$fecha_licencia', observaciones = '$observaciones' WHERE IDempleado = '$el_empleado'";
mysql_select_db($database_vacantes, $vacantes);
...
$insertSQL = "INSERT INTO pc_semaforo (IDempleado, IDmatriz, IDpuesto, IDpuestoPC, fecha_licencia, estatus, reqa, reqb, reqc, reqd, reqe, reqf, observaciones) VALUES ('$el_empleado', '$IDmatriz', '$IDpuesto', '$IDpuestoPC', '$fecha_licencia','$estatus', '$reqa', '$reqb', '$reqc', '$reqd', '$reqe', '$reqf', '$observaciones')";
mysql_select_db($database_vacantes, $vacantes);
			...
			<!-- Content area -->
				...
				<div class="content">
                  ...
                  <div class="panel panel-flat">
					...
					<div class="panel-body">
							<p>Bienvenido. El inventario de personal permite identificar al personal que cuenta con los requisitos necesarios para ser considerado en el Plan de Carrera.</br>
					 		   En el inventario, se muestran los empleados activos con <strong>prioridad al plan de Carrera para el puesto de Chofer</strong>.</br>
                               Recuerda que debes mantener actualizado el inventario de personal y evaluar a todos los empleados.</p>
                            ...
                            <p><a href="plan_carr_reporte.php" class="btn btn-success">Descargar reporte detallado</a></p>

                       <form method="POST" action="plan_carrera_inv.php">
					...
					<table class="table">
						<tbody>							  
                            ...
                            <?php do { ?>
                               <option value="<?php echo $row_puestos['IDpuesto']?>"<?php if (!(strcmp($row_puestos['IDpuesto'], $el_puesto))) {echo "selected=\"selected\"";} ?>>
                               <?php echo $row_puestos['denominacion']?></option>
                               <?php
                              } while ($row_puestos = mysql_fetch_assoc($puestos));
                              $rows = mysql_num_rows($puestos);
                              if($rows > 0) {
                                  mysql_data_seek($puestos, 0);
                                  $row_area = mysql_fetch_assoc($puestos);
                              } ?> </select>
						    </td>
							<td>
				...
				<div class="table-responsive">
                    ...
                    <tbody>
						<?php do {  ?>
                        <tr>
                            <td><?php echo $row_detalle['IDempleado']; $row_detalle['IDplan'];?>&nbsp; </td>
                            ...
                            <td><?php 
								...
								elseif ($row_detalle['reqb'] == 0 OR  $row_detalle['reqe'] == 0) {echo "No viable";} 
