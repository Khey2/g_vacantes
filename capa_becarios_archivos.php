...
mysql_select_db($database_vacantes, $vacantes);
$query_becarios  = "SELECT capa_becarios_files.file AS Archivo, vac_areas.area, capa_becarios_files.*, capa_becarios.*, capa_becarios_tipo.tipo, capa_becarios_tipo_file.tipo_file FROM capa_becarios_files LEFT JOIN capa_becarios ON  capa_becarios_files.IDempleado = capa_becarios.IDempleado LEFT JOIN capa_becarios_tipo_file ON capa_becarios_files.IDtipo_file = capa_becarios_tipo_file.IDtipo_file LEFT JOIN capa_becarios_tipo ON capa_becarios.IDtipo = capa_becarios_tipo.IDtipo LEFT JOIN vac_areas ON capa_becarios.IDarea = vac_areas.IDarea WHERE capa_becarios_files.borrado = 0";
...
if ((isset($_GET['borrar'])) && ($_GET['borrar'] == 1)) {
  ...
  header("Location: capa_becarios_archivos.php?info=3");
}
...
$query_becaries = "SELECT * FROM capa_becarios";
$becaries = mysql_query($query_becaries, $vacantes) or die(mysql_error());
$row_becaries = mysql_fetch_assoc($becaries);
$totalRows_becaries = mysql_num_rows($becaries);
...
header("Location: capa_becarios_archivos.php?info=9"); 
exit;
}
...
$query2 = "INSERT INTO capa_becarios_files (IDempleado, IDmes, anio, file, fecha, IDtipo_file, borrado) VALUES ('$ELempleado', '$IDmes', '$IDanio', '$name_new', '$fecha', '$IDtipo_file', 0)"; 
$result2 = mysql_query($query2) or die(mysql_error());  
header("Location: capa_becarios_archivos.php?info=1"); 
...
<body <?php if (isset($_COOKIE["lmenu"])) { echo 'class="sidebar-xs"';}?>>
	...
	<div class="media panel-body no-margin">
		...
		<p>Selecciona la evaluación para Consultar.</p>
						...
						<?php do { ?>
							<tr>
							<td><?php echo $row_becarios['IDfile']; ?></td>
							...
							echo $elmes; ?></td>
							<td><?php echo $row_becarios['tipo_file']; ?></td>
							<td>
								<a href="becariosfiles/<?php echo $row_becarios['IDempleado']; ?>/<?php echo $row_becarios['Archivo']; ?>" class="btn btn-primary">Descargar</a>
								<button type="button" data-target="#modal_theme_danger<?php echo $row_becarios['IDfile']; ?>"  data-toggle="modal" class="btn btn-danger">Borrar</button>
								</td>
						    </tr>
							
							
					<!-- danger modal -->
					<div id="modal_theme_danger<?php echo $row_becarios['IDfile']; ?>" class="modal fade" tabindex="-1">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
                                    ...
                                    <a class="btn btn-danger" href="capa_becarios_archivos.php?IDfile=<?php echo $row_becarios['IDfile']; ?>&borrar=1">Si borrar</a>
								</div>
							</div>
						</div>
					</div>
										...
										<div class="modal-dialog">
											<div class="modal-content">
												...
												<div class="modal-body">
														...
														<form action="capa_becarios_archivos.php" method="post" name="importar" id="importar" class="form-horizontal" enctype="multipart/form-data">
														 <fieldset>
														<p>&nbsp;</p>

														<!-- Fecha -->
														<div class="form-group">
															<label class="control-label col-lg-3">Becario:<span class="text-danger">*</span></label>
														<div class="col-lg-9">
																<select name="IDempleado" id="IDempleado" class="bootstrap-select" data-live-search="true" data-width="100%" required="required">
																	<option value="">Seleccione una opción</option> 
																		  <?php do {  ?>
																		  <option value="<?php echo $row_becaries['IDempleado']?>"><?php echo $row_becaries['emp_paterno']." ".$row_becaries['emp_materno']." ".$row_becaries['emp_nombre']?></option>
																		  <?php
																		 } while ($row_becaries = mysql_fetch_assoc($becaries));
																		 $rows = mysql_num_rows($becaries);
																		 if($rows > 0) {
																		 mysql_data_seek($becaries, 0);
																		 $row_becaries = mysql_fetch_assoc($becaries);
																		 } ?>
															  </select>
													   </div>
														...
														<div class="col-lg-9">
																<select name="IDmes" id="IDmes" class="bootstrap-select" data-live-search="true" data-width="100%" required="required">
																	<option value="">Seleccione una opción</option> 
																		  <?php do {  ?>
