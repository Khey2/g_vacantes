...
mysql_select_db($database_vacantes, $vacantes);
$query_seman = "SELECT DISTINCT prod_captura.semana,  prod_captura.anio, prod_captura.IDmatriz FROM prod_captura WHERE prod_captura.IDmatriz = '$IDmatriz' AND semana > 0 AND prod_captura.anio = $el_anio GROUP BY prod_captura.semana, prod_captura.anio";
...
//Detalle
mysql_select_db($database_vacantes, $vacantes);
$query_detalle = "SELECT vac_matriz.region_op, Count( prod_activos.IDempleado ) AS Ocupados, Count( prod_captura.capturador ) AS TCapturados, Count( prod_captura.validador ) AS TValidados, Count( prod_captura.autorizador ) AS TAutorizados, Sum( prod_captura.pago_total ) AS TTotal, Sum( prod_captura.adicional2 ) AS TAdicional, Sum( prod_captura.sueldo_total ) AS TSueldo, vac_matriz.matriz, vac_matriz.IDmatriz, vac_matriz.incluye_pull, vac_puestos.IDpuesto FROM prod_activos LEFT JOIN vac_puestos ON prod_activos.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_matriz ON prod_activos.IDmatriz = vac_matriz.IDmatriz LEFT JOIN prod_captura ON prod_captura.IDempleado = prod_activos.IDempleado WHERE vac_puestos.IDaplica_PROD = 1 AND prod_activos.IDmatriz = $IDmatriz GROUP BY prod_activos.IDmatriz";  
								...
								<div class="panel-body">
									<div class="tabbable">
										...
										<div class="tab-content">
                                            ...
                                            <p>A continuación se muestan el comprativo de gasto:<br />
                                            Selecciona el año en el siguiente filtro:</p>
                                            
                    <form method="POST" action="prod_comparativo_s.php">
                            <div class="col-lg-3">
                            ...
                            <button type="submit" class="btn btn-primary">Filtrar <i class="icon-arrow-right14 position-right"></i></button>
											...
											<table class="table table-condensed datatable-button-html5-columns">
												...
												mysql_select_db($database_vacantes, $vacantes);
												$query_presupuesto = "SELECT Count(prod_plantilla.IDplantilla) AS Autorizada, prod_plantilla.IDpuesto, prod_plantilla.IDmatriz, prod_plantilla.IDsucursal, prod_plantilla.IDtipo_plaza, prod_plantilla.IDestatus, prod_plantilla.fecha_inicio, prod_plantilla.fecha_fin, prod_plantilla.fecha_congelada, vac_puestos.denominacion, vac_puestos.dias, prod_garantias.monto_asistencia, vac_areas.IDarea, vac_areas.area, vac_matriz.matriz, prod_garantias.garantia, vac_tabulador.sueldo_diario FROM prod_plantilla LEFT JOIN vac_tabulador ON prod_plantilla.IDpuesto = vac_tabulador.IDpuesto AND vac_tabulador.IDmatriz = '$IDmatriz' LEFT JOIN prod_garantias ON prod_plantilla.IDpuesto = prod_garantias.IDpuesto AND prod_garantias.IDmatriz = '$IDmatriz' LEFT JOIN vac_puestos ON vac_puestos.IDpuesto = prod_plantilla.IDpuesto LEFT JOIN vac_areas ON vac_puestos.IDarea = vac_areas.IDarea LEFT JOIN vac_matriz ON prod_plantilla.IDmatriz = vac_matriz.IDmatriz WHERE prod_plantilla.IDmatriz = '$IDmatriz' AND vac_puestos.IDaplica_PROD = 1 AND vac_puestos.IDarea IN (1,2,3,4) AND prod_plantilla.IDestatus = 1 AND prod_plantilla.IDtipo_plaza in (".$pulls.") AND (DATE(fecha_inicio) <= '$fecha_filtro') AND ( DATE(fecha_fin) > '$fecha_filtro' OR DATE(fecha_fin) = '0000-00-00' OR DATE(fecha_fin) IS NULL) AND ( DATE(fecha_congelada) > '$fecha_filtro' OR DATE(fecha_congelada) = '0000-00-00' OR DATE(fecha_congelada) IS NULL) GROUP BY prod_plantilla.IDpuesto, prod_plantilla.IDmatriz, prod_plantilla.IDtipo_plaza ORDER BY vac_puestos.denominacion ASC";
												...
												mysql_select_db($database_vacantes, $vacantes);
												$query_adicional = "SELECT prod_meses_presupesto_adicional.*, SUM(prod_meses_presupesto_adicional.resultado) as resultadoP  FROM prod_meses_presupesto_adicional WHERE IDmatriz = '$IDmatriz' AND IDanio = '$el_anio' AND IDsemana = $semana AND IDestatus = 1";
												...
												// presupuesto directo
												$Total0 = 0; 
												do {$Total0 = $Total0 + (((($row_presupuesto['garantia'] * ($row_presupuesto['sueldo_diario']) * 7) / 100 ) + $row_presupuesto['monto_asistencia']) * $row_presupuesto['Autorizada']); } while ($row_presupuesto = mysql_fetch_assoc($presupuesto)); 
												...
												?>
                                                  <tr>
													...
													<td><span class="text <?php if( $rebase == 1){ echo "text-danger"; } ?>"><strong></strong> <?php echo "$" . number_format(round($Total0, 2));?></span></td>
                                                  </tr>
