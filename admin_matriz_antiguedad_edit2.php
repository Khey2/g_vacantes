...
mysql_select_db($database_vacantes, $vacantes);
$query_autorizados = "SELECT prod_valor_antiguedad.*, vac_matriz.matriz, vac_puestos.denominacion, vac_areas.area FROM prod_valor_antiguedad LEFT JOIN vac_matriz ON prod_valor_antiguedad.IDmatriz = vac_matriz.IDmatriz LEFT JOIN vac_puestos ON prod_valor_antiguedad.IDpuesto = vac_puestos.IDpuesto LEFT JOIN vac_areas ON vac_puestos.IDarea = vac_areas.IDarea WHERE prod_valor_antiguedad.IDvalor = $IDvalor";
...
$updateSQL = "UPDATE prod_valor_antiguedad SET IDnivel='$IDnivel', meses_inicio=$meses_inicio, meses_final=$meses_final WHERE IDvalor = $IDvalor";
mysql_select_db($database_vacantes, $vacantes);
$Result1 = mysql_query($updateSQL, $vacantes) or die(mysql_error());
header('Location: admin_matriz_antiguedad_edit.php?IDmatriz='.$IDmatriz.'&info=2');
}
...
$meses_inicio = $_POST["meses_inicio"];
$meses_final = $_POST["meses_final"];
$insertSQL = "INSERT INTO prod_valor_antiguedad (IDmatriz, IDpuesto, IDnivel, meses_inicio, meses_final) VALUES ($IDmatriz, $IDpuesto, '$IDnivel', $meses_inicio, $meses_final)";
mysql_select_db($database_vacantes, $vacantes);
$Result1 = mysql_query($insertSQL, $vacantes) or die(mysql_error());
header('Location: admin_matriz_antiguedad_edit.php?IDmatriz='.$IDmatriz.'&info=1');
}
...
if ((isset($_GET['borrar'])) && ($_GET['borrar'] == 1)) {
    ...
    $deleteSQL = "DELETE FROM  prod_valor_antiguedad WHERE IDvalor ='$IDvalor'";
  
  mysql_select_db($database_vacantes, $vacantes);
  $result = mysql_query($deleteSQL, $vacantes) or die(mysql_error());
  header('Location: admin_matriz_antiguedad_edit.php?IDmatriz='.$IDmatriz.'&info=3');
}
...
mysql_select_db($database_vacantes, $vacantes);
$query_puestos = "SELECT * FROM vac_puestos WHERE IDaplica_PROD = 1 ORDER BY denominacion ASC";
...
<div class="content">
                  ...
                  <div class="panel panel-flat">
					...
					<div class="panel-body">
                            ...
                            <form method="post" name="form1" action="admin_matriz_antiguedad_edit2.php?IDmatriz=<?php echo $IDmatriz; ?>" class="form-horizontal form-validate-jquery">
									...
									<!-- Basic select -->
									<div class="form-group">
										<label class="control-label col-lg-3">Denominacion:<span class="text-danger">*</span></label>
										<div class="col-lg-9">
												<?php  if (isset($_GET["IDvalor"])) { ?>
											<input type="text" name="IDpuesto" id="IDpuesto" class="form-control" value="<?php echo htmlentities($row_autorizados['denominacion'], ENT_COMPAT, ''); ?>" required="required">
												  <?php } else { ?>
                              ...
                              <?php } else { ?>
                                ...
                                <?php }  ?>
                                    <button type="button" onClick="window.location.href='admin_matriz_antiguedad_edit.php'" class="btn btn-default btn-icon">Cancelar</button>
                            </form>
                            <p>&nbsp;</p>
