...
mysql_select_db($database_vacantes, $vacantes);
$query_catalogo = "SELECT capa_eventos_grupo_matriz.*, vac_matriz.matriz FROM capa_eventos_grupo_matriz INNER JOIN vac_matriz ON capa_eventos_grupo_matriz.IDmatriz = vac_matriz.IDmatriz WHERE IDCcurso = $IDCcurso";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO capa_eventos_grupo_matriz (IDusuario, IDCcurso, IDmatriz) VALUES (%s, %s, %s)",
                       GetSQLValueString($IDusuario, "int"),
                       GetSQLValueString($IDCcurso, "int"),
                       GetSQLValueString($_POST['IDmatriz'], "int"));

  mysql_select_db($database_vacantes, $vacantes);
  $Result1 = mysql_query($insertSQL, $vacantes) or die(mysql_error());
  $captura = mysql_insert_id();
  header("Location: capa_catalogos_9.php?info=1&IDCcurso=$IDCcurso"); 	
}

// borrar alternativo
if ((isset($_GET['borrar'])) && ($_GET['borrar'] == 1)) {
  
  $borrado = $_GET['IDCcurso_tipos'];
  $deleteSQL = "DELETE FROM capa_eventos_grupo_matriz WHERE IDCcurso_tipos = $borrado";

  mysql_select_db($database_vacantes, $vacantes);
  $result = mysql_query($deleteSQL, $vacantes) or die(mysql_error());
  header("Location: capa_catalogos_9.php?info=3&IDCcurso=$IDCcurso");
}

// agregar todos
if ((isset($_GET['todos'])) && ($_GET['todos'] == 1)) {
$query_puestosall = "SELECT IDmatriz FROM vac_matriz WHERE IDmatriz NOT IN (27,31)";
$puestosall = mysql_query($query_puestosall, $vacantes) or die(mysql_error());
$row_puestosall = mysql_fetch_assoc($puestosall);

do {
$elpuesto = $row_puestosall['IDmatriz'];
$agregartodos = "INSERT INTO capa_eventos_grupo_matriz (IDCcurso, IDmatriz, IDusuario) value ('$IDCcurso', '$elpuesto', '$IDusuario')";
mysql_select_db($database_vacantes, $vacantes);
...
$deleteSQL = "DELETE FROM capa_eventos_grupo_matriz WHERE IDCcurso = $borrado";
mysql_select_db($database_vacantes, $vacantes);
$result = mysql_query($deleteSQL, $vacantes) or die(mysql_error());
header("Location: capa_catalogos_9.php?info=3&IDCcurso=$IDCcurso");
}
...
$borrado = $_GET['IDCcurso'];
foreach ($_POST['IDmatriz'] as $credens) { $credenciales = implode(",", $_POST['IDmatriz']); } echo $credenciales;
$deleteSQL = "DELETE FROM capa_eventos_grupo_matriz WHERE IDCcurso = $borrado AND IDmatriz IN ($credenciales)";
mysql_select_db($database_vacantes, $vacantes);
$result = mysql_query($deleteSQL, $vacantes) or die(mysql_error());
header("Location: capa_catalogos_9.php?info=3&IDCcurso=$IDCcurso");
}
...
$query_el_curso = "SELECT * FROM capa_eventos_grupo WHERE IDCcurso = $IDCcurso";
$el_curso = mysql_query($query_el_curso, $vacantes) or die(mysql_error());
$row_el_curso = mysql_fetch_assoc($el_curso);

$query_puesto = "SELECT * FROM vac_matriz WHERE IDmatriz NOT IN (27,31) ORDER BY matriz ASC";
				...
				<div class="content">
                  ...
                  <div class="panel panel-flat">
					...
					<div class="panel-body">
                      ...
                      <?php } ?>
						<a href="capa_catalogos_6.php" class="btn btn-default">Regresar</a><p>&nbsp;</p>
						
						
						
        <form method="post" name="form1" action="capa_catalogos_9.php?todos=3&IDCcurso=<?php echo $IDCcurso; ?>">
                      ...
                      <?php do {  ?>
                        <tr>
                          <td><input class="control-warning" type="checkbox" name="IDmatriz[]" value="<?php echo $row_catalogo['IDmatriz']; ?>"></td>
                          <td><?php echo $row_el_curso['evento']; ?></td>
                          <td><?php echo $row_catalogo['IDmatriz']; ?></td>
                          <td><?php echo $row_catalogo['matriz']; ?></td>
                         <td>
						 <button type="button" data-target="#modal_theme_danger<?php echo $row_catalogo['IDCcurso_tipos']; ?>"  data-toggle="modal" class="btn bg-danger">Borrar</button>
						 </td>
                        </tr> 


                     <!-- danger modal -->
					<div id="modal_theme_danger<?php echo $row_catalogo['IDCcurso_tipos']; ?>" class="modal fade" tabindex="-1">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
                                    ...
                                    <a class="btn btn-danger" href="capa_catalogos_9.php?IDCcurso_tipos=<?php echo $row_catalogo['IDCcurso_tipos']; ?>&IDCcurso=<?php echo $IDCcurso; ?>&borrar=1">Si borrar</a>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<form action="capa_catalogos_9.php?IDCcurso=<?php echo $IDCcurso; ?>" method="post" name="importar" id="importar" class="form-horizontal" enctype="multipart/form-data">
									<fieldset>
											...
											<div class="col-lg-8">
												<select name="IDmatriz" id="IDmatriz" class="form-control" required="required">
													<option value="">Seleccione una opción</option> 
														  <?php do {  ?>
														  <option value="<?php echo $row_puesto['IDmatriz']?>"><?php echo $row_puesto['matriz']?></option>
														  <?php
					...
					<div id="modal_theme_agregar2" class="modal fade" tabindex="-1">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                                    <a href="capa_catalogos_9.php?todos=1&IDCcurso=<?php echo $IDCcurso; ?>" class="btn btn-danger">Si Agregar</a>
								</div>
							</div>
						</div>
					</div>
					...
					<div id="modal_theme_agregar3" class="modal fade" tabindex="-1">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                                    <a href="capa_catalogos_9.php?todos=2&IDCcurso=<?php echo $IDCcurso; ?>" class="btn btn-danger">Si Borrar</a>
								</div>
							</div>
						</div>
					</div>
